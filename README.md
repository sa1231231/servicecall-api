...
5. **Ingests leads** from a Google Sheet (Apps Script), enriches them via Anthropic web_search/web_fetch + Google Places (text search + phone-number lookup) + Brave Search, and lets the operator promote any lead into a real agent in one click.
...
Pipeline: **Apps Script → POST /api/leads/intake → Mongo (queued) → background enrichment → ready/failed → operator promote → real agent.** Enrichment runs an Anthropic skill (`skills/onboarding-to-config/`) with `web_search` + `web_fetch` tool use, augmented by Google Places (text search + phone-number lookup) and Brave Search (if configured) as pre-searches.
...
| **Brave Search** | Long-tail web context (Yelp pages, Nextdoor, BBB) for enrichment | `BRAVE_API_KEY` |
