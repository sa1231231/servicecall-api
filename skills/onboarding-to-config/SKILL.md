...
2. **`## Google Custom Search results`** — general web hits (Yelp, Nextdoor, Facebook, BBB, contractor-locator pages, the business's own site, news). Use for hours, services, summaries that Places didn't surface.
...
3. **If Places is empty, check Custom Search.** A hit in the right area code (973 = NJ, 415 = SF, etc.) with a plausibly-related name is a usable match — just flag the verification gap inside the FAQ.
4. **Cross-reference for FAQ details.** Hours from Places. Services from Custom Search snippets / business website / fetched listing pages. Reviews-derived "what they're known for" from either.

### Falling back to web_search

When **both** Places and Custom Search blocks are empty (or every hit is irrelevant — wrong city, wrong vertical, generic directories with no business name), **call `web_search`** before considering DRAFT. Try the phone number in multiple formats:
...
Reserve the DRAFT path **only when Places, Custom Search, AND your `web_search` calls all came back empty across every query**. If any source produced a relevant hit, commit to that business in the JSON — even if some details (hours, full address) need verification, just note the gaps in the FAQ. DRAFT is for "we found nothing across every channel"; partial info is not DRAFT.

If both pre-search blocks are missing entirely (rare — would indicate a backend bug), still try `web_search` before giving up. Only output a DRAFT with the lead-form `name` as `businessName` and `templateName: ""` after the tool also turns up nothing.
