...
Every prompt for this skill arrives with up to two pre-search blocks generated by our backend before the model is invoked:
...
2. **`## Google Custom Search results`** *(may be absent)* — general web hits (Yelp, Nextdoor, Facebook, BBB, contractor-locator pages, the business's own site, news). When present, use for hours, services, summaries that Places didn't surface. The Custom Search block is **omitted entirely** when the deployment hasn't configured a Programmable Search Engine — that's expected, not a backend bug. Fall through to `web_search` when it's missing or thin.
