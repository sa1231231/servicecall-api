...
Every prompt for this skill arrives with up to four pre-search blocks generated by our backend before the model is invoked. Strongest signal first:
...
2. **`## Yelp phone-search results`** *(may be absent)* — Yelp Fusion phone-match. Strong signal for B2B service businesses (handyman, HVAC, plumbing, etc.) — Yelp's listings are well-curated and the categories field maps directly to a service vertical. When Yelp returns a hit, treat the listing name as canonical. **Yelp resolves the name and categories only — it never carries the business's own website** (the URL shown is the yelp.com page). A Yelp hit alone is therefore never "the full picture." Block is omitted when YELP_API_KEY isn't configured.
...
**2. Find the official website.** The business's own website is the single richest FAQ source, so resolving it is a priority — not optional. The Places block carries it on a `website:` line when Google has it; **Yelp never does** (its URL is the yelp.com page). If no `website:` appears in pre-search, `web_search` `"<business name> <city>"` and read the results for the official site or the business's Facebook page (`facebook.com/...` — its About section lists the website and phone; cross-check that phone against the lead).

**3. Mine the website for the FAQ.** Once the official site is known, **`web_fetch` it** — it is the primary FAQ source. Pull policies (cancellation, warranty, service guarantees, payment/financing terms), services, hours, service area, and maintenance plans. Fetch it even when Places/Yelp already named the business — those carry metadata, not the business's own policy and service detail. The Facebook and Yelp pages are secondary sources to fill any gaps.
...
- The business is fully resolved **and** its website is already confirmed **and** Places/Yelp gave you concrete services + hours — i.e. there is genuinely nothing left to fetch. A resolved business whose website is still unknown is **not** "complete" — fetch to find and mine it.
