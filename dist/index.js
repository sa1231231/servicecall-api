...
app.listen(Number(config.PORT), () => {
    ...
    (async () => {
        try {
            const result = await getDb().collection("users").updateOne({ _id: "sam_admin", role: { $ne: "super_admin" } }, { $set: { role: "super_admin", permissions: {
            ...
            if (result.modifiedCount > 0)
                console.log("[users] promoted sam_admin to super_admin");
        }
        catch (err) {
            console.warn("[users] sam_admin promotion failed:", err);
        }
    })();
