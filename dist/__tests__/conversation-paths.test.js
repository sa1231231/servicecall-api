...
import { startChat, sendUserMessage, endChat, createChatCloneFromVoiceAgent, deleteChatAgent, } from "../lib/retell-chat-driver.js";
import { AGENT_FIXTURES } from "./_fixtures/index.js";
const BASE_URL = process.env.SYSTEM_TEST_URL ?? process.env.BASE_URL;
const API_KEY = process.env.API_KEY;
const ADMIN_PASSWORD = process.env.ADMIN_PASSWORD;
const RETELL_API_KEY = process.env.RETELL_API_KEY;
const hasConfig = !!BASE_URL && BASE_URL.startsWith("http") && !!API_KEY && !!RETELL_API_KEY && !!ADMIN_PASSWORD;
...
async function fetchPathInfoMap(slug, agentId) {
    const resp = await fetch(url(`/dashboard/api/agents/${slug}/nodes/${agentId}`), { headers: authHeaders() });
    if (!resp.ok)
        throw new Error(`Failed to fetch path info for ${slug}: ${resp.status}`);
    ...
    for (const p of body.paths ?? []) {
        const collectNodeNameToVar = {};
        const dataPointOrder = [];
        for (const dp of p.dataPoints ?? []) {
            const nodeName = nodeIdToName[dp.collectNodeId];
            if (nodeName)
                collectNodeNameToVar[nodeName] = dp.variableName;
...
async function runScenario(fixture, chatAgentId, scenario, pathInfo) {
    ...
    for (let i = 0; i < maxTurns; i++) {
        ...
        if (allAnswered && currentNode && TERMINAL_NODE_RE.test(currentNode))
            break;
...
function matchesExpectVariables(run, scenario) {
 ...
 */
async function runScenarioMajorityPass(fixture, chatAgentId, scenario, pathInfo, N = 3) {
    const passing = [];
    const failures = [];
    for (let i = 0; i < N; i++) {
        try {
            const r = await runScenario(fixture, chatAgentId, scenario, pathInfo);
            if (matchesExpectVariables(r, scenario))
                passing.push(r);
            else
                failures.push({ run: r });
        }
    ...
    if (passing.length < required) {
        throw new Error(`majority-pass failed for ${scenario.scenarioName}: ${passing.length}/${N} passed (needed ${required})`);
    }
    return passing[0];
}
async function fetchCallLog(slug, callId, attempts = 8) {
    for (let i = 0; i < attempts; i++) {
        const resp = await fetch(url(`/dashboard/api/agents/${slug}/calls?limit=20&include_tests=1`), { headers: authHeaders() });
        if (resp.ok) {
            const calls = await resp.json();
            const match = calls.find((c) => c._id === callId);
            if (match)
                return match;
        }
...
function debugDump(run, scenario, pathInfo) {
    const lines = [];
    lines.push("");
...
describe.skipIf(!hasConfig)("Conversation routing paths (live Retell chat)", { timeout: 240_000 }, () => {
    for (const fixture of AGENT_FIXTURES) {
        describe(`agent: ${fixture.slug}`, () => {
            ...
            beforeAll(async () => {
                ...
                chatAgentId = await createChatCloneFromVoiceAgent(fixture.agentId, `chat-clone-test-${fixture.slug}-${Date.now()}`);
            });
            afterAll(async () => {
                if (chatAgentId) {
                    try {
            ...
            for (const scenario of fixture.scenarios) {
                describe(`${scenario.pathName} / ${scenario.scenarioName}`, () => {
                    ...
                    it(`drives chat (${scenario.description})`, async () => {
                        ...
                        if (!pathInfo)
                            throw new Error(`Path ${scenario.pathName} not found in deployed flow for ${fixture.slug}`);
                        run = scenario.flaky
            ...
            it("every deployed path has at least one fixture scenario", async () => {
                const deployedPaths = new Set(Object.keys(pathInfoMap));
