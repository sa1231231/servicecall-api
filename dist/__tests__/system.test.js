...
describe.skipIf(!hasConfig)("System tests (Railway)", { timeout: 30_000 }, () => {
    ...
    describe("Dashboard API", () => {
        it("lists agents including demo client", async () => {
            ...
            const demo = body.find((a) => a.slug === DEMO_SLUG);
    ...
    describe("QA smoke test", () => {
        ...
        it("agent is reachable in Retell", () => {
            const check = smokeReport?.checks?.find((c) => c.check === "agent_reachable");
            expect(check?.status).toBe("pass");
        });
        it("data points match between flow and notification config", () => {
            const check = smokeReport?.checks?.find((c) => c.check === "data_points_in_flow");
            expect(check?.status).toBe("pass");
        });
        it("notification config is complete", () => {
            const check = smokeReport?.checks?.find((c) => c.check === "notification_config_complete");
            expect(check?.status).toBe("pass");
        });
        it("message type resolves correctly", () => {
            const check = smokeReport?.checks?.find((c) => c.check === "message_type_resolves");
            expect(check?.status).toBe("pass");
        });
    });
