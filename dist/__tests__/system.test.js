...
describe.skipIf(!hasConfig)("System tests (Railway)", { timeout: 30_000 }, () => {
    ...
    describe("QA smoke test", () => {
        ...
        it("runs smoke test", async () => {
            ...
            const checkNames = smokeReport.checks.map((c) => c.check);
        ...
        it("agent is reachable", () => {
            expect(smokeReport?.checks?.find((c) => c.check === "agent_reachable")?.status).toBe("pass");
        });
    });
    // ── 8. Weekly Reports ──────────────────────────────────────────────────
    ...
    describe("Portal API", () => {
        ...
        it("GET /portal/:slug/api/agent includes dispatch_call_overrides when present", async () => {
            ...
            if (resp.status !== 200)
                return; // J&A may not exist in all environments
            const body = await json(resp);
    ...
    describe("Roles and permissions", () => {
        ...
        it("sam_admin exists and has elevated permissions", async () => {
            ...
            const sam = users.find((u) => u._id === "sam_admin");
            if (sam) {
        ...
        it("settings include view_billing and manage_deleted in permission defs", async () => {
            ...
            const body = await json(resp);
            const keys = body.permissionDefs.map((d) => d.key);
    ...
    describe("Export agent config", () => {
        ...
        it("exported paths match node editor structure", async () => {
            ...
            // Path names match
            const exportNames = config.paths.map((p) => p.name).sort();
            const nodeNames = nodeData.paths.map((p) => p.name).sort();
            expect(exportNames).toEqual(nodeNames);
            // Data point counts match per path
            for (const ep of config.paths) {
                const np = nodeData.paths.find((p) => p.name === ep.name);
                if (np) {
    ...
    describe("Node editor", () => {
        let preEditSnapshotId;
        let initialDontMeasureVars = [];
        // ── GET structure ──────────────────────────────────────────────
        describe("GET node structure", () => {
            it("returns structured tree for multi-path agent with branches", async () => {
                ...
                const mm = body.paths.find((p) => p.name === "measure_me");
                const dm = body.paths.find((p) => p.name === "dont_measure_me");
                expect(mm).toBeDefined();
                expect(dm).toBeDefined();
                // measure_me data points
                expect(mm.dataPoints.length).toBeGreaterThanOrEqual(8);
                const mmVars = mm.dataPoints.map((d) => d.variableName);
                ...
                const prefDay = mm.dataPoints.find((d) => d.variableName === "preferred_day");
                expect(prefDay.branchConditions).toBeDefined();
                expect(prefDay.branchConditions.some((c) => c.variable === "property_type" && c.operator === "==")).toBe(true);
                const warranty = mm.dataPoints.find((d) => d.variableName === "warranty_status");
                expect(warranty.branchConditions).toBeDefined();
                expect(warranty.branchConditions.some((c) => c.variable === "payment_method" && c.operator === "!=")).toBe(true);
                // dont_measure_me
                const dmVars = dm.dataPoints.map((d) => d.variableName);
        ...
        describe("save-and-publish integration", () => {
            it("adds email to dont_measure_me, publishes, and verifies", { timeout: 45_000 }, async () => {
                // Get current state
                const struct = await json(await fetch(url(`/dashboard/api/agents/${SLUG}/nodes/${AGENT_ID}`), { headers: authHeaders() }));
                const dm = struct.paths.find((p) => p.name === "dont_measure_me");
                const currentVars = dm.dataPoints.map((d) => d.variableName);
                ...
                const dm2 = after.paths.find((p) => p.name === "dont_measure_me");
                expect(dm2.dataPoints.map((d) => d.variableName)).toContain("email");
                // measure_me should be unaffected
                const mm = after.paths.find((p) => p.name === "measure_me");
                expect(mm.dataPoints.map((d) => d.variableName)).toContain("property_type");
            });
            it("removes email and reorders, publishes, and verifies", { timeout: 45_000 }, async () => {
                const struct = await json(await fetch(url(`/dashboard/api/agents/${SLUG}/nodes/${AGENT_ID}`), { headers: authHeaders() }));
                const dm = struct.paths.find((p) => p.name === "dont_measure_me");
                const withoutEmail = dm.dataPoints
                    .filter((d) => d.variableName !== "email")
                ...
                const after = await json(await fetch(url(`/dashboard/api/agents/${SLUG}/nodes/${AGENT_ID}`), { headers: authHeaders() }));
                const dm2 = after.paths.find((p) => p.name === "dont_measure_me");
                expect(dm2.dataPoints.map((d) => d.variableName)).not.toContain("email");
            });
        ...
        describe("Live integration (publish → verify in Retell → rollback)", () => {
            ...
            it("adds email to dont_measure_me and publishes to Retell", { timeout: 45_000 }, async () => {
                ...
                const dm = before.paths.find((p) => p.name === "dont_measure_me");
                const currentVars = dm.dataPoints.map((d) => d.variableName);
                const resp = await fetch(url(`/dashboard/api/agents/${SLUG}/nodes/${AGENT_ID}/save-and-publish`), {
                    method: "POST", headers: authHeaders(),
                    body: JSON.stringify({
            ...
            it("reorders dont_measure_me and verifies in Retell", { timeout: 45_000 }, async () => {
                const before = await json(await fetch(url(`/dashboard/api/agents/${SLUG}/nodes/${AGENT_ID}`), { headers: authHeaders() }));
                const dm = before.paths.find((p) => p.name === "dont_measure_me");
                const vars = dm.dataPoints.map((d) => d.variableName);
                ...
                const after = await json(await fetch(url(`/dashboard/api/agents/${SLUG}/nodes/${AGENT_ID}`), { headers: authHeaders() }));
                const dm2 = after.paths.find((p) => p.name === "dont_measure_me");
                const afterVars = dm2.dataPoints.map((d) => d.variableName);
            ...
            it("removes email and verifies in Retell", { timeout: 45_000 }, async () => {
                const before = await json(await fetch(url(`/dashboard/api/agents/${SLUG}/nodes/${AGENT_ID}`), { headers: authHeaders() }));
                const dm = before.paths.find((p) => p.name === "dont_measure_me");
                const withoutEmail = dm.dataPoints.filter((d) => d.variableName !== "email").map((d) => d.variableName);
                const resp = await fetch(url(`/dashboard/api/agents/${SLUG}/nodes/${AGENT_ID}/save-and-publish`), {
                    method: "POST", headers: authHeaders(),
                    body: JSON.stringify({
                        changes: {
            ...
            it("adds a new path and verifies in Retell", { timeout: 45_000 }, async () => {
                ...
                const newPath = after.paths.find((p) => p.name === "test_path_integration");
                expect(newPath).toBeDefined();
                expect(newPath.dataPoints.map((d) => d.variableName)).toContain("full_name");
                expect(newPath.dataPoints.map((d) => d.variableName)).toContain("email");
            ...
            it("measure_me branches are intact after all edits", async () => {
                ...
                expect(mm).toBeDefined();
                expect(mm.dataPoints.map((d) => d.variableName)).toContain("property_type");
                expect(mm.dataPoints.map((d) => d.variableName)).toContain("warranty_status");
                expect(mm.dataPoints.map((d) => d.variableName)).toContain("truck_number");
                // Branch conditions should still exist
                const prefDay = mm.dataPoints.find((d) => d.variableName === "preferred_day");
                if (prefDay) {
                    expect(prefDay.branchConditions).toBeDefined();
                    expect(prefDay.branchConditions.some((c) => c.variable === "property_type")).toBe(true);
                }
            });
            ...
            it("version history has entries from integration tests", async () => {
                ...
                const descriptions = body.versions.map((v) => v.description);
                expect(descriptions.some((d) => d.includes("Integration"))).toBe(true);
            });
            ...
            it("agent is clean after rollback — 2 paths, no test_path", async () => {
                const body = await json(await fetch(url(`/dashboard/api/agents/${SLUG}/nodes/${AGENT_ID}`), { headers: authHeaders() }));
                expect(body.paths.length).toBe(2);
                expect(body.paths.some((p) => p.name === "test_path_integration")).toBe(false);
                expect(body.paths.some((p) => p.name === "measure_me")).toBe(true);
                expect(body.paths.some((p) => p.name === "dont_measure_me")).toBe(true);
    ...
    describe("Display name (Retell sync)", { timeout: 30_000 }, () => {
        ...
        it("dashboard list reflects the new display_name", async () => {
            ...
            const row = list.find((a) => a.slug === SLUG);
    ...
    describe("Rename business (script-only contract)", { timeout: 60_000 }, () => {
        ...
        it("rename rewrites script + Mongo without reporting nickname work", async () => {
            ...
            const resp = await fetch(url(`/dashboard/api/agents/${SLUG}/nodes/${AGENT_ID}/rename-business`), {
                method: "POST", headers: authHeaders(),
        ...
        afterAll(async () => {
            ...
            if (!originalName)
                return;
            const doc = await json(await fetch(url(`/dashboard/api/agents/${SLUG}`), { headers: authHeaders() }));
            if (doc.name === originalName)
                return;
            await fetch(url(`/dashboard/api/agents/${SLUG}/nodes/${AGENT_ID}/rename-business`), {
                method: "POST", headers: authHeaders(),
    ...
    describe("Folders CRUD + agent move", { timeout: 30_000 }, () => {
        ...
        it("lists the new folder", async () => {
            ...
            expect(folders.some((f) => f._id === testFolderId)).toBe(true);
        });
