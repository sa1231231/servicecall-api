...
import { requireRole, requirePermission, requireRoot, requireRootForProtectedSlug, ROOT_ONLY_DELETE_SLUGS, } from "../require-role.js";
...
function mockReqWithSlug(slug, user) {
    return { params: { slug }, user };
}
describe("requireRootForProtectedSlug", () => {
    ...
    it("blocks non-root admin from deleting demo-meter", () => {
        const next = vi.fn();
        const res = mockRes();
        requireRootForProtectedSlug(mockReqWithSlug("demo-meter", makeUser({ role: "admin", isRoot: false })), res, next);
        expect(next).not.toHaveBeenCalled();
        expect(res.status).toHaveBeenCalledWith(403);
    });
    it("blocks non-root operator from deleting demo-meter", () => {
        const next = vi.fn();
        const res = mockRes();
        requireRootForProtectedSlug(mockReqWithSlug("demo-meter", makeUser({ role: "operator", isRoot: false })), res, next);
        expect(next).not.toHaveBeenCalled();
        expect(res.status).toHaveBeenCalledWith(403);
    });
    it("allows root user to delete demo-meter", () => {
        const next = vi.fn();
        const res = mockRes();
        requireRootForProtectedSlug(mockReqWithSlug("demo-meter", makeUser({ isRoot: true })), res, next);
        expect(next).toHaveBeenCalled();
    ...
    it("allows non-root super_admin to delete a non-protected slug", () => {
        const next = vi.fn();
        const res = mockRes();
        requireRootForProtectedSlug(mockReqWithSlug("acme", makeUser({ role: "super_admin", isRoot: false })), res, next);
        expect(next).toHaveBeenCalled();
        expect(res.status).not.toHaveBeenCalled();
    });
    ...
    it("passes through when slug param is missing and slug is not protected", () => {
        const next = vi.fn();
        const res = mockRes();
        requireRootForProtectedSlug({ params: {}, user: makeUser({ isRoot: false }) }, res, next);
        expect(next).toHaveBeenCalled();
    });
});
