...
emailNotificationRouter.post("/", async (req, res) => {
    ...
    try {
        ...
        const company_name = collectedVars?.company_name ?? dynamicVars?.company_name ?? "";
        const full_name = collectedVars?.full_name ?? dynamicVars?.full_name ?? "";
        const raw_phone = collectedVars?.phone_number ?? dynamicVars?.phone_number ?? "";
        const phone_number = raw_phone && raw_phone !== "Not Mentioned"
            ? raw_phone
            : (parsedEvent?.call?.from_number ?? "");
        const truck_number = collectedVars?.truck_number ?? dynamicVars?.truck_number ?? "";
        const driver_name = collectedVars?.driver_name ?? dynamicVars?.driver_name ?? "";
        const driver_phone = collectedVars?.driver_phone ?? dynamicVars?.driver_phone ?? "";
        const breakdown_location = collectedVars?.breakdown_location ??
            dynamicVars?.breakdown_location ??
            "";
        const problem_description = collectedVars?.problem_description ??
            dynamicVars?.problem_description ??
            "";
        const whos_paying = collectedVars?.whos_paying ?? dynamicVars?.whos_paying ?? "";
