...
const { mockGetClientDocument, mockLoadClientsFromDb, mockGetDb, mockUpdateOne, mockFetchRetellAgent, mockExtractVariables, mockParseConversationFlow, mockValidateConversationFlow, mockCreateVersionSnapshot, mockLogAudit, mockDeriveNotificationConfig, mockGetDataPointDefaults, mockResolveDataPoints, mockRegenerateDataChain, mockApplyRegeneratedChain, mockPushFlowToRetell, mockGetWarmTransferAgentVersion, mockBuildWarmTransferOption, mockRenderTemplate, } = vi.hoisted(() => ({
...
vi.mock("../../../lib/audit.js", () => ({ logAudit: (...a) => mockLogAudit(...a) }));
...
vi.mock("../../../middleware/require-role.js", () => ({
    requireRoot: (req, res, next) => next(),
}));
vi.mock("../../../lib/node-regenerator.js", () => ({
    regenerateDataChain: (...a) => mockRegenerateDataChain(...a),
    applyRegeneratedChain: (...a) => mockApplyRegeneratedChain(...a),
}));
...
vi.mock("../../../lib/agent-generator/generate-agent.js", () => ({
    resolveDataPoints: (...a) => mockResolveDataPoints(...a),
}));
...
vi.mock("../../../lib/agent-generator/node-builders.js", () => ({
    ...
    buildWarmTransferOption: (...a) => mockBuildWarmTransferOption(...a),
...
vi.mock("../../../lib/build-notification.js", () => ({
    renderTemplate: (...a) => mockRenderTemplate(...a),
}));
...
beforeEach(() => {
    ...
    mockRenderTemplate.mockImplementation((tpl) => tpl);
});
// ── edit-branch-condition ──────────────────────────────────────────────────
describe("POST /:agentId/edit-branch-condition", () => {
    it("returns 400 when variableName missing", async () => {
        const res = makeRes();
        await runRoute("post", "/:agentId/edit-branch-condition", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { branchConditions: [] } }), res);
        expect(res._status).toBe(400);
    });
    it("returns 400 when branchConditions is neither null nor an array", async () => {
        const res = makeRes();
        await runRoute("post", "/:agentId/edit-branch-condition", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { variableName: "x", branchConditions: "wat" } }), res);
        expect(res._status).toBe(400);
    });
    it("returns 404 when variable not in path", async () => {
        ...
        const res = makeRes();
        await runRoute("post", "/:agentId/edit-branch-condition", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { variableName: "missing", branchConditions: null } }), res);
    ...
    it("removes branch condition when null and audits", async () => {
        ...
        const res = makeRes();
        await runRoute("post", "/:agentId/edit-branch-condition", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { variableName: "full_name", branchConditions: null } }), res);
        expect(res._status).toBe(200);
        expect(res._json).toEqual({ success: true, variableName: "full_name", pathName: "Default" });
        expect(mockLogAudit).toHaveBeenCalledWith(expect.anything(), "edit_branch_condition", "acme/agent_1", { variableName: "full_name", pathName: "Default", branchConditions: null });
    });
    it("sets branch conditions when array provided", async () => {
        ...
        const res = makeRes();
        await runRoute("post", "/:agentId/edit-branch-condition", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: {
                variableName: "email",
...
describe("POST /:agentId/edit-path-name", () => {
    it("returns 400 when oldName or newName missing", async () => {
        const res = makeRes();
        await runRoute("post", "/:agentId/edit-path-name", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { oldName: "Old" } }), res);
        expect(res._status).toBe(400);
    });
    it("short-circuits to success when oldName === newName", async () => {
        const res = makeRes();
        await runRoute("post", "/:agentId/edit-path-name", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { oldName: "Same", newName: "Same" } }), res);
        expect(res._status).toBe(200);
    ...
    it("returns 404 when path not found", async () => {
        ...
        const res = makeRes();
        await runRoute("post", "/:agentId/edit-path-name", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { oldName: "Sales", newName: "Leads" } }), res);
        expect(res._status).toBe(404);
    });
    it("renames nodes, message_types, default_message_type, dispatch_by_type, path_end_modes, and audits", async () => {
        ...
        expect(nodes[0].variables[0].description).toBe('Always set to "Leads".');
        ...
        expect(mockLogAudit).toHaveBeenCalledWith(expect.anything(), "edit_path_name", "acme/agent_1", { oldName: "Sales", newName: "Leads" });
    });
});
...
describe("POST /:agentId/edit-human-request-mode", () => {
    it("returns 400 when mode not callback/live_transfer", async () => {
        const res = makeRes();
        await runRoute("post", "/:agentId/edit-human-request-mode", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { mode: "weird" } }), res);
        expect(res._status).toBe(400);
    });
    it("returns 404 when agent not found", async () => {
        mockGetClientDocument.mockResolvedValue(null);
        const res = makeRes();
        await runRoute("post", "/:agentId/edit-human-request-mode", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { mode: "callback" } }), res);
        expect(res._status).toBe(404);
    });
    ...
    it("switches to callback: removes Transfer Call node, sets callback edge", async () => {
        ...
        expect(nodes.find((n) => n.name === "Transfer Call")).toBeUndefined();
        expect(nodes.find((n) => n.name === "Live Transfer Recovery")).toBeUndefined();
        expect(mockLogAudit).toHaveBeenCalledWith(expect.anything(), "edit_human_request_mode", "acme/agent_1", { mode: "callback" });
    });
    it("switches to live_transfer: adds Transfer Call + Live Transfer Recovery", async () => {
        ...
        mockParseConversationFlow.mockReturnValue({ paths: [] });
        const res = makeRes();
        await runRoute("post", "/:agentId/edit-human-request-mode", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { mode: "live_transfer" } }), res);
        expect(res._status).toBe(200);
        expect(nodes.find((n) => n.name === "Transfer Call")).toBeDefined();
        expect(nodes.find((n) => n.name === "Live Transfer Recovery")).toBeDefined();
    ...
    it("migrates legacy 'Transfer Failed' name to 'Live Transfer Recovery'", async () => {
        ...
        expect(nodes.find((n) => n.name === "Transfer Failed")).toBeUndefined();
        expect(nodes.find((n) => n.name === "Live Transfer Recovery")).toBeDefined();
    });
});
// ── edit-path-end-mode ─────────────────────────────────────────────────────
describe("POST /:agentId/edit-path-end-mode", () => {
    it("returns 400 when pathName missing", async () => {
        const res = makeRes();
        await runRoute("post", "/:agentId/edit-path-end-mode", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { mode: "callback" } }), res);
        expect(res._status).toBe(400);
    });
    it("returns 400 when mode not callback/transfer", async () => {
        const res = makeRes();
        await runRoute("post", "/:agentId/edit-path-end-mode", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { pathName: "Sales", mode: "x" } }), res);
        expect(res._status).toBe(400);
    });
    it("returns 404 when agent not found", async () => {
        mockGetClientDocument.mockResolvedValue(null);
        const res = makeRes();
        await runRoute("post", "/:agentId/edit-path-end-mode", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { pathName: "Sales", mode: "callback" } }), res);
        expect(res._status).toBe(404);
    });
    it("returns 400 for transfer mode when no dispatch number is configured", async () => {
        ...
        const res = makeRes();
        await runRoute("post", "/:agentId/edit-path-end-mode", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { pathName: "Sales", mode: "transfer" } }), res);
        expect(res._status).toBe(400);
    ...
    it("switches a multi-path callback toggle creates a per-path Close, pushes flow, persists path_end_modes, audits", async () => {
        ...
        const newClose = nodes.find((n) => n.name === "Close (Sales)");
        ...
        expect(nodes.find((n) => n.name === "Pre-Transfer (Sales)")).toBeUndefined();
        expect(nodes.find((n) => n.name === "Transfer Call (Sales)")).toBeUndefined();
    ...
    it("single-path callback toggle reuses the singleton Close node", async () => {
        ...
        const res = makeRes();
        await runRoute("post", "/:agentId/edit-path-end-mode", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { pathName: "Solo", mode: "callback" } }), res);
        expect(res._status).toBe(200);
        // Single-path: rewires to the singleton "Close" node — no per-path clone.
        expect(router.else_edge.destination_node_id).toBe("close");
        expect(nodes.find((n) => n.name === "Close (Solo)")).toBeUndefined();
    });
    it("transfer toggle removes the path's per-path Close node", async () => {
        const doc = makeDoc({ dispatch_call_number: "+15555" });
        mockGetClientDocument.mockResolvedValue(doc);
        const router = { id: "router-Sales", else_edge: { destination_node_id: "close-Sales" } };
        const nodes = [
            router,
        ...
        await runRoute("post", "/:agentId/edit-path-end-mode", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { pathName: "Sales", mode: "transfer" } }), res);
        expect(res._status).toBe(200);
        expect(nodes.find((n) => n.name === "Close (Sales)")).toBeUndefined();
        // Other path's Close node is preserved.
        expect(nodes.find((n) => n.name === "Close (Other)")).toBeDefined();
    });
    it("switches a path to transfer: creates Pre-Transfer + Transfer Call + recovery", async () => {
        const doc = makeDoc({ dispatch_call_number: "+15555" });
        mockGetClientDocument.mockResolvedValue(doc);
        const router = { id: "router-Sales", else_edge: { destination_node_id: "close" } };
        const nodes = [router, { id: "close", name: "Close" }, { id: "cr", name: "Closing Remarks" }];
        mockFetchRetellAgent.mockResolvedValue({
            canonicalJson: { conversationFlow: { nodes } }, conversationFlowId: "f1", agentName: "A",
        });
        mockParseConversationFlow.mockReturnValue({
        ...
        await runRoute("post", "/:agentId/edit-path-end-mode", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { pathName: "Sales", mode: "transfer" } }), res);
        expect(res._status).toBe(200);
        expect(nodes.find((n) => n.name === "Pre-Transfer (Sales)")).toBeDefined();
        expect(nodes.find((n) => n.name === "Transfer Call (Sales)")).toBeDefined();
        expect(nodes.find((n) => n.name === "Live Transfer Recovery")).toBeDefined();
    ...
    it("uses per-path dispatch override when present", async () => {
        ...
        mockGetClientDocument.mockResolvedValue(doc);
        const router = { id: "router-Sales", else_edge: { destination_node_id: "close" } };
        const nodes = [router, { id: "close", name: "Close" }];
        mockFetchRetellAgent.mockResolvedValue({
            canonicalJson: { conversationFlow: { nodes } }, conversationFlowId: "f1", agentName: "A",
        });
        mockParseConversationFlow.mockReturnValue({
        ...
        const tc = nodes.find((n) => n.name === "Transfer Call (Sales)");
        expect(tc.transfer_destination.number).toBe("+19999");
    });
});
