...
vi.mock("../../../config/client-store.js", () => ({
    ...
    listDeletedClients: (...a) => mockListDeletedClients(...a),
    restoreClient: (...a) => mockRestoreClient(...a),
    deleteClient: (...a) => mockDeleteClient(...a),
}));
vi.mock("../../../lib/call-log.js", () => ({ getCallLogById: (...a) => mockGetCallLogById(...a) }));
vi.mock("../../../lib/notify-sms.js", () => ({ sendSmsToAll: (...a) => mockSendSmsToAll(...a) }));
vi.mock("../../../lib/settings.js", () => ({
    getSettings: (...a) => mockGetSettings(...a),
    updateSettings: (...a) => mockUpdateSettings(...a),
}));
vi.mock("../../../lib/backup.js", () => ({ runBackup: (...a) => mockRunBackup(...a) }));
vi.mock("../../../lib/data-point-defaults.js", () => ({
    getDataPointDefaultsWithCategory: (...a) => mockGetDataPointDefaultsWithCategory(...a),
    updateDataPointDefault: (...a) => mockUpdateDataPointDefault(...a),
    createDataPointDefault: (...a) => mockCreateDataPointDefault(...a),
    deleteDataPointDefault: (...a) => mockDeleteDataPointDefault(...a),
    reorderDataPointDefaults: (...a) => mockReorderDataPointDefaults(...a),
    CATEGORY_ORDER: ["A", "B"],
...
vi.mock("../../../middleware/require-role.js", () => ({
    ...
    requireRoot: (_req, _res, next) => next(),
    requireRootForProtectedSlug: (_req, _res, next) => next(),
}));
...
vi.mock("../../../lib/root-alerts.js", () => ({
    alertRootIfNeeded: (...a) => mockAlertRootIfNeeded(...a),
}));
vi.mock("../../../lib/users.js", () => ({
    listUsers: (...a) => mockListUsers(...a),
    createUser: (...a) => mockCreateUser(...a),
    deleteUser: (...a) => mockDeleteUser(...a),
    updateUserPermissions: (...a) => mockUpdateUserPermissions(...a),
    PERMISSION_DEFS: [],
    DEFAULT_PERMISSIONS: {},
}));
vi.mock("../../../lib/billing-cogs.js", () => ({ getClientCogs: (...a) => mockGetClientCogs(...a) }));
vi.mock("../../../lib/blast-sms.js", () => ({
    previewBlast: (...a) => mockPreviewBlast(...a),
    sendBlast: (...a) => mockSendBlast(...a),
}));
...
function makeRes() {
    ...
    res.send = (data) => { res._data = data; return res; };
...
describe("POST /deleted-agents/:slug/restore", () => {
    it("strips [DELETED — expires ...] suffix from Retell agent name and audits", async () => {
        ...
        const res = makeRes();
        await runRoute(dashboardApiRouter, "post", "/deleted-agents/:slug/restore", makeReq({ params: { slug: "acme" } }), res);
...
describe("GET /agents/:slug/calls/:callId/transcript", () => {
    it("returns 404 when call not found or wrong slug", async () => {
        ...
        const res = makeRes();
        await runRoute(dashboardApiRouter, "get", "/agents/:slug/calls/:callId/transcript", makeReq({ params: { slug: "acme", callId: "c1" } }), res);
...
describe("GET /agents/:slug/portal-token", () => {
    it("returns has_token: false when no token exists", async () => {
        mockGetClientDocument.mockResolvedValue({});
        const res = makeRes();
        await runRoute(dashboardApiRouter, "get", "/agents/:slug/portal-token", makeReq({ params: { slug: "acme" } }), res);
    ...
    it("builds portal URL from request when token exists", async () => {
        ...
        const res = makeRes();
        await runRoute(dashboardApiRouter, "get", "/agents/:slug/portal-token", makeReq({ params: { slug: "acme" }, host: "app.example.com" }), res);
...
describe("POST /agents/:slug/portal-token", () => {
    it("returns 404 when client missing", async () => {
        ...
        const res = makeRes();
        await runRoute(dashboardApiRouter, "post", "/agents/:slug/portal-token", makeReq({ params: { slug: "acme" } }), res);
        expect(res._status).toBe(404);
    });
    it("generates token and returns URL", async () => {
        ...
        const res = makeRes();
        await runRoute(dashboardApiRouter, "post", "/agents/:slug/portal-token", makeReq({ params: { slug: "acme" }, host: "x.com" }), res);
...
describe("POST /agents/:slug/request-review", () => {
    it("returns 404 when client missing", async () => {
        mockGetClientDocument.mockResolvedValue(null);
        const res = makeRes();
        await runRoute(dashboardApiRouter, "post", "/agents/:slug/request-review", makeReq({ params: { slug: "acme" } }), res);
        expect(res._status).toBe(404);
    });
...
describe("POST /agents/:slug/send-payment-link", () => {
    it("templates stripe_payment_url and sends", async () => {
        ...
        mockSendSmsToAll.mockResolvedValue(undefined);
        const res = makeRes();
        await runRoute(dashboardApiRouter, "post", "/agents/:slug/send-payment-link", makeReq({ params: { slug: "acme" } }), res);
        expect(res._status).toBe(200);
...
describe("POST /agents/:slug/send-portal-link", () => {
    it("returns 400 when no portal token yet", async () => {
        mockGetClientDocument.mockResolvedValue({ dispatch_text_numbers: ["+1"] });
        const res = makeRes();
        await runRoute(dashboardApiRouter, "post", "/agents/:slug/send-portal-link", makeReq({ params: { slug: "acme" } }), res);
        expect(res._status).toBe(400);
    ...
    it("templates the portal URL and sends", async () => {
        ...
        mockSendSmsToAll.mockResolvedValue(undefined);
        const res = makeRes();
        await runRoute(dashboardApiRouter, "post", "/agents/:slug/send-portal-link", makeReq({ params: { slug: "acme" }, host: "app.x.com" }), res);
        expect(res._status).toBe(200);
...
describe("GET /billing/cogs/:slug", () => {
    it("clamps months to [1, 24]", async () => {
        ...
        const res = makeRes();
        await runRoute(dashboardApiRouter, "get", "/billing/cogs/:slug", makeReq({ params: { slug: "acme" }, query: { months: "100" } }), res);
        expect(mockGetClientCogs).toHaveBeenCalledWith("acme", 24);
    });
    it("defaults to 6 months when not provided", async () => {
        mockGetClientCogs.mockResolvedValue({ months: [] });
        const res = makeRes();
        await runRoute(dashboardApiRouter, "get", "/billing/cogs/:slug", makeReq({ params: { slug: "acme" } }), res);
...
describe("GET / PATCH /settings", () => {
    ...
    it("updates settings and audits", async () => {
        ...
        const res = makeRes();
        await runRoute(dashboardApiRouter, "patch", "/settings", makeReq({ body: { foo: "bar" } }), res);
        expect(res._status).toBe(200);
...
describe("POST /blast-sms/preview", () => {
    it("returns 400 when message missing", async () => {
        const res = makeRes();
        await runRoute(dashboardApiRouter, "post", "/blast-sms/preview", makeReq({ body: {} }), res);
        expect(res._status).toBe(400);
    });
    it("returns preview from previewBlast", async () => {
        mockPreviewBlast.mockReturnValue({ recipients: 5 });
        const res = makeRes();
        await runRoute(dashboardApiRouter, "post", "/blast-sms/preview", makeReq({ body: { message: "hi" } }), res);
...
describe("POST /blast-sms", () => {
    ...
    it("returns 400 when message exceeds 1600 chars", async () => {
        const res = makeRes();
        await runRoute(dashboardApiRouter, "post", "/blast-sms", makeReq({ body: { message: "x".repeat(1601) } }), res);
        expect(res._status).toBe(400);
    });
    it("sends and audits", async () => {
        mockSendBlast.mockResolvedValue({ total_recipients: 5, total_clients: 2, sent: 5, failed: [] });
        const res = makeRes();
        await runRoute(dashboardApiRouter, "post", "/blast-sms", makeReq({ body: { message: "hi all" } }), res);
        expect(res._status).toBe(200);
        expect(res._json.success).toBe(true);
...
describe("PATCH /data-point-defaults/:key", () => {
    it("returns 404 when not found", async () => {
        ...
        const res = makeRes();
        await runRoute(dashboardApiRouter, "patch", "/data-point-defaults/:key", makeReq({ params: { key: "missing" }, body: {} }), res);
        expect(res._status).toBe(404);
    });
    it("returns updated dp on success", async () => {
        mockUpdateDataPointDefault.mockResolvedValue({ key: "k", label: "L" });
        const res = makeRes();
        await runRoute(dashboardApiRouter, "patch", "/data-point-defaults/:key", makeReq({ params: { key: "k" }, body: { label: "L" } }), res);
...
describe("POST /data-point-defaults", () => {
    it("returns 400 when key/label missing", async () => {
        const res = makeRes();
        await runRoute(dashboardApiRouter, "post", "/data-point-defaults", makeReq({ body: {} }), res);
        expect(res._status).toBe(400);
    });
    it("creates dp and audits", async () => {
        mockCreateDataPointDefault.mockResolvedValue({ key: "new" });
        const res = makeRes();
        await runRoute(dashboardApiRouter, "post", "/data-point-defaults", makeReq({ body: { key: "new", label: "New", type: "string" } }), res);
        expect(res._json.success).toBe(true);
    ...
    it("returns 400 when create throws", async () => {
        ...
        const res = makeRes();
        await runRoute(dashboardApiRouter, "post", "/data-point-defaults", makeReq({ body: { key: "k", label: "L" } }), res);
        expect(res._status).toBe(400);
    });
});
describe("PUT /data-point-defaults/reorder", () => {
    it("returns 400 when items not array", async () => {
        const res = makeRes();
        await runRoute(dashboardApiRouter, "put", "/data-point-defaults/reorder", makeReq({ body: {} }), res);
        expect(res._status).toBe(400);
    });
    it("reorders successfully", async () => {
        mockReorderDataPointDefaults.mockResolvedValue(undefined);
        const res = makeRes();
        await runRoute(dashboardApiRouter, "put", "/data-point-defaults/reorder", makeReq({ body: { items: [{ key: "a" }, { key: "b" }] } }), res);
        expect(res._json.success).toBe(true);
    });
});
describe("DELETE /data-point-defaults/:key", () => {
    it("returns 404 when not found", async () => {
        ...
        const res = makeRes();
        await runRoute(dashboardApiRouter, "delete", "/data-point-defaults/:key", makeReq({ params: { key: "x" } }), res);
        expect(res._status).toBe(404);
    });
...
describe("POST /users", () => {
    it("returns 400 when username/password missing", async () => {
        const res = makeRes();
        await runRoute(dashboardApiRouter, "post", "/users", makeReq({ body: {}, user: { username: "admin" } }), res);
        expect(res._status).toBe(400);
    });
    it("returns 400 for invalid username characters", async () => {
        const res = makeRes();
        await runRoute(dashboardApiRouter, "post", "/users", makeReq({
...
describe("PATCH /users/:username/permissions", () => {
    it("returns 400 when permissions object missing", async () => {
        const res = makeRes();
        await runRoute(dashboardApiRouter, "patch", "/users/:username/permissions", makeReq({ params: { username: "alice" }, body: {} }), res);
        expect(res._status).toBe(400);
    });
    it("returns 404 when user not found", async () => {
        mockUpdateUserPermissions.mockResolvedValue(false);
        const res = makeRes();
        await runRoute(dashboardApiRouter, "patch", "/users/:username/permissions", makeReq({ params: { username: "alice" }, body: { permissions: { x: true } } }), res);
        expect(res._status).toBe(404);
    });
...
describe("DELETE /users/:username", () => {
    it("returns 400 when deleting self", async () => {
        const res = makeRes();
        await runRoute(dashboardApiRouter, "delete", "/users/:username", makeReq({ params: { username: "admin" }, user: { username: "admin" } }), res);
        expect(res._status).toBe(400);
    });
    it("returns 404 when user not found", async () => {
        mockDeleteUser.mockResolvedValue(false);
        const res = makeRes();
        await runRoute(dashboardApiRouter, "delete", "/users/:username", makeReq({ params: { username: "ghost" }, user: { username: "admin" } }), res);
        expect(res._status).toBe(404);
    });
    it("deletes and audits", async () => {
        mockDeleteUser.mockResolvedValue(true);
        const res = makeRes();
        await runRoute(dashboardApiRouter, "delete", "/users/:username", makeReq({ params: { username: "alice" }, user: { username: "admin" } }), res);
        expect(res._json.success).toBe(true);
        expect(mockLogAudit).toHaveBeenCalled();
    });
});
