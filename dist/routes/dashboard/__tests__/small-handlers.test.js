...
vi.mock("../../../lib/retell-display-sync.js", () => ({
    syncRetellDisplayLabels: (...a) => mockSyncRetellDisplayLabels(...a),
}));
...
function makeReq(opts) {
    return { params: opts.params ?? {}, body: opts.body ?? {}, query: opts.query ?? {} };
}
beforeEach(() => {
    for (const m of [
...
describe("updateAgentHandler", () => {
    ...
    it("does not call Retell sync when display_name is not in the body", async () => {
        ...
        const res = makeRes();
        await updateAgentHandler(makeReq({ params: { slug: "acme" }, body: { shadow_mode: true } }), res);
        expect(res._status).toBe(200);
    ...
    it("returns 502 when Retell sync fails after the Mongo write", async () => {
        ...
        const res = makeRes();
        await updateAgentHandler(makeReq({ params: { slug: "acme" }, body: { display_name: "Acme (demo)" } }), res);
        expect(res._status).toBe(502);
        expect(res._json.error).toMatch(/retell down/);
    });
    it("rejects non-string non-null display_name with 400", async () => {
        const res = makeRes();
        await updateAgentHandler(makeReq({ params: { slug: "acme" }, body: { display_name: 42 } }), res);
        expect(res._status).toBe(400);
    ...
    describe("contact_* field normalization", () => {
        it("trims whitespace-only contact_name to null", async () => {
            ...
            mockGetClientDocument.mockResolvedValue({ name: "Acme" });
            const res = makeRes();
            await updateAgentHandler(makeReq({ params: { slug: "acme" }, body: { contact_name: "   " } }), res);
            expect(res._status).toBe(200);
        ...
        it("trims surrounding whitespace on contact_email and saves", async () => {
            ...
            mockGetClientDocument.mockResolvedValue({ name: "Acme" });
            const res = makeRes();
            await updateAgentHandler(makeReq({ params: { slug: "acme" }, body: { contact_email: "  ops@x.com  " } }), res);
            expect(res._status).toBe(200);
        ...
        it("rejects non-string non-null contact_phone with 400", async () => {
            const res = makeRes();
            await updateAgentHandler(makeReq({ params: { slug: "acme" }, body: { contact_phone: 5551234567 } }), res);
            expect(res._status).toBe(400);
        ...
        it("passes contact_timezone through unchanged when valid", async () => {
            ...
            mockGetClientDocument.mockResolvedValue({ name: "Acme" });
            const res = makeRes();
            await updateAgentHandler(makeReq({ params: { slug: "acme" }, body: { contact_timezone: "America/Chicago" } }), res);
            expect(res._status).toBe(200);
        ...
        it("converts empty-string contact_notes to null", async () => {
            ...
            mockGetClientDocument.mockResolvedValue({ name: "Acme" });
            const res = makeRes();
            await updateAgentHandler(makeReq({ params: { slug: "acme" }, body: { contact_notes: "" } }), res);
            expect(res._status).toBe(200);
        ...
        it("accepts explicit null on any contact_* field", async () => {
            ...
            mockGetClientDocument.mockResolvedValue({ name: "Acme" });
            const res = makeRes();
            await updateAgentHandler(makeReq({ params: { slug: "acme" }, body: { contact_name: null } }), res);
            expect(res._status).toBe(200);
            expect(mockUpdateClientFields).toHaveBeenCalledWith("acme", { contact_name: null });
        });
    });
