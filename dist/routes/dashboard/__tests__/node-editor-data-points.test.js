...
function makeDp(name, label = name) {
    return {
        variableName: name,
        label,
...
function makePath(name, dpNames) {
    return {
        name,
...
describe("POST /:agentId/add-data-point", () => {
    it("returns 400 when dataPointKey missing", async () => {
        const res = makeRes();
        await runRoute("post", "/:agentId/add-data-point", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: {} }), res);
        expect(res._status).toBe(400);
    });
    it("returns 404 when agent not found", async () => {
        mockGetClientDocument.mockResolvedValue(null);
        const res = makeRes();
        await runRoute("post", "/:agentId/add-data-point", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { dataPointKey: "email" } }), res);
    ...
    it("returns 400 when dataPointKey is unknown", async () => {
        ...
        const res = makeRes();
        await runRoute("post", "/:agentId/add-data-point", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { dataPointKey: "bogus" } }), res);
        expect(res._status).toBe(400);
    ...
    it("returns 404 when path is not found", async () => {
        ...
        const res = makeRes();
        await runRoute("post", "/:agentId/add-data-point", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { dataPointKey: "email", pathName: "Sales" } }), res);
        expect(res._status).toBe(404);
    ...
    it("returns 400 when variable already exists in path", async () => {
        ...
        const res = makeRes();
        await runRoute("post", "/:agentId/add-data-point", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { dataPointKey: "full_name" } }), res);
        expect(res._status).toBe(400);
    ...
    it("inserts at end when position omitted, audits success", async () => {
        ...
        expect(mockLogAudit).toHaveBeenCalledWith(expect.anything(), "add_data_point", "acme/agent_1", expect.objectContaining({ dataPointKey: "email", pathName: "Default", position: 1 }));
    });
    it("clamps position to valid range when given out-of-bounds", async () => {
        ...
        const res = makeRes();
        await runRoute("post", "/:agentId/add-data-point", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { dataPointKey: "email", position: 99 } }), res);
        expect(res._status).toBe(200);
...
describe("POST /:agentId/remove-data-point", () => {
    it("returns 400 when variableName missing", async () => {
        const res = makeRes();
        await runRoute("post", "/:agentId/remove-data-point", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: {} }), res);
        expect(res._status).toBe(400);
    });
    it("returns 404 when agent not found", async () => {
        mockGetClientDocument.mockResolvedValue(null);
        const res = makeRes();
        await runRoute("post", "/:agentId/remove-data-point", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { variableName: "email" } }), res);
        expect(res._status).toBe(404);
    });
    ...
    it("returns 400 when removing the last data point", async () => {
        ...
        const res = makeRes();
        await runRoute("post", "/:agentId/remove-data-point", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { variableName: "full_name" } }), res);
        expect(res._status).toBe(400);
    ...
    it("removes successfully and audits", async () => {
        ...
        expect(mockLogAudit).toHaveBeenCalledWith(expect.anything(), "remove_data_point", "acme/agent_1", { variableName: "email", pathName: "Default" });
    });
});
// ── reorder-data-points ────────────────────────────────────────────────────
describe("POST /:agentId/reorder-data-points", () => {
    it("returns 400 when variableNames not an array", async () => {
        const res = makeRes();
        await runRoute("post", "/:agentId/reorder-data-points", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { variableNames: "x" } }), res);
        expect(res._status).toBe(400);
    });
    it("returns 400 when variableNames is empty", async () => {
        const res = makeRes();
        await runRoute("post", "/:agentId/reorder-data-points", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { variableNames: [] } }), res);
        expect(res._status).toBe(400);
    });
    it("returns 404 when agent not found", async () => {
        mockGetClientDocument.mockResolvedValue(null);
        const res = makeRes();
        await runRoute("post", "/:agentId/reorder-data-points", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { variableNames: ["a"] } }), res);
        expect(res._status).toBe(404);
    });
    it("returns 400 with missing/extra when names don't match path", async () => {
        ...
        const res = makeRes();
        await runRoute("post", "/:agentId/reorder-data-points", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { variableNames: ["full_name", "phone"] } }), res);
        expect(res._status).toBe(400);
    ...
    it("reorders successfully and audits", async () => {
        ...
        const res = makeRes();
        await runRoute("post", "/:agentId/reorder-data-points", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { variableNames: ["email", "full_name"] } }), res);
        expect(res._status).toBe(200);
        expect(res._json).toEqual({ success: true, variableNames: ["email", "full_name"], pathName: "Default" });
        expect(mockLogAudit).toHaveBeenCalledWith(expect.anything(), "reorder_data_points", "acme/agent_1", { variableNames: ["email", "full_name"], pathName: "Default" });
    });
});
