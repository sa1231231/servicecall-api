...
vi.mock("../../../lib/agent-versions.js", () => ({
    ...
    getVersion: (...a) => mockGetVersion(...a),
    listVersions: (...a) => mockListVersions(...a),
    getLatestVersion: (...a) => mockGetLatestVersion(...a),
}));
...
describe("POST /:agentId/rollback", () => {
    ...
    it("pushes flow, updates agent settings, and creates two version snapshots on success", async () => {
        ...
        expect(res._json.agentSettingsUpdated).toEqual(expect.arrayContaining(["agent_name", "voice_id"]));
        expect(mockPushFlowToRetell).toHaveBeenCalledTimes(1);
        expect(mockAgentUpdate).toHaveBeenCalledWith("agent_1", expect.objectContaining({ agent_name: "Old Name", voice_id: "voice_old" }));
        // Pre-rollback + post-rollback snapshots
        expect(mockCreateVersionSnapshot).toHaveBeenCalledTimes(2);
        expect(mockLogAudit).toHaveBeenCalledWith(expect.anything(), "rollback_agent", "acme/agent_1", expect.objectContaining({ restoredVersion: 5, versionId: "v123" }));
    });
...
describe("POST /:agentId/push (raw JSON)", () => {
    ...
    it("snapshots, pushes, re-fetches, snapshots again, and audits on success", async () => {
        ...
        expect(res._status).toBe(200);
        expect(res._json.success).toBe(true);
        expect(mockPushFlowToRetell).toHaveBeenCalledWith(expect.anything(), "flow_1", newJson);
        expect(mockCreateVersionSnapshot).toHaveBeenCalledTimes(2);
        ...
        expect(mockLogAudit).toHaveBeenCalledWith(expect.anything(), "push_raw_json", "acme/agent_1", { description: "manual fix" });
    });
