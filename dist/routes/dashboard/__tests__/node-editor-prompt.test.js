...
describe("POST /:agentId/edit-prompt", () => {
    it("returns 400 when nodeId is missing", async () => {
        const res = makeRes();
        await runRoute("post", "/:agentId/edit-prompt", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { instruction: "hi" } }), res);
        expect(res._status).toBe(400);
    });
    it("returns 400 when instruction is empty", async () => {
        const res = makeRes();
        await runRoute("post", "/:agentId/edit-prompt", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { nodeId: "n1", instruction: "  " } }), res);
        expect(res._status).toBe(400);
    });
    it("returns 404 when agent not found", async () => {
        mockGetClientDocument.mockResolvedValue(null);
        const res = makeRes();
        await runRoute("post", "/:agentId/edit-prompt", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { nodeId: "n1", instruction: "ok" } }), res);
    ...
    it("returns 404 when target node is missing in flow", async () => {
        ...
        const res = makeRes();
        await runRoute("post", "/:agentId/edit-prompt", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { nodeId: "missing", instruction: "ok" } }), res);
        expect(res._status).toBe(404);
    ...
    it("returns 400 when validation fails after edit", async () => {
        ...
        const res = makeRes();
        await runRoute("post", "/:agentId/edit-prompt", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { nodeId: "n1", instruction: "new prompt" } }), res);
        expect(res._status).toBe(400);
    ...
    it("snapshots, applies edit, stores, and audits on success", async () => {
        ...
        const res = makeRes();
        await runRoute("post", "/:agentId/edit-prompt", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { nodeId: "n1", instruction: "new prompt" }, username: "alice" }), res);
        ...
        expect(mockLogAudit).toHaveBeenCalledWith(expect.anything(), "edit_node_prompt", "acme/agent_1", { nodeId: "n1", nodeName: "Greeting" });
...
describe("POST /:agentId/edit-global-prompt", () => {
    it("returns 400 when globalPrompt missing or empty", async () => {
        const res = makeRes();
        await runRoute("post", "/:agentId/edit-global-prompt", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { globalPrompt: " " } }), res);
        expect(res._status).toBe(400);
    });
    it("returns 404 when agent not found", async () => {
        mockGetClientDocument.mockResolvedValue(null);
        const res = makeRes();
        await runRoute("post", "/:agentId/edit-global-prompt", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { globalPrompt: "p" } }), res);
        expect(res._status).toBe(404);
    });
    it("returns 400 when validation fails", async () => {
        ...
        mockValidateConversationFlow.mockReturnValue([{ code: "X", message: "bad" }]);
        const res = makeRes();
        await runRoute("post", "/:agentId/edit-global-prompt", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { globalPrompt: "new" } }), res);
        expect(res._status).toBe(400);
        expect(res._json.error).toBe("Validation failed");
    });
    it("applies edit and audits on success", async () => {
        ...
        const res = makeRes();
        await runRoute("post", "/:agentId/edit-global-prompt", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { globalPrompt: "new global" }, username: "bob" }), res);
        expect(res._status).toBe(200);
...
describe("POST /:agentId/edit-transition", () => {
    it("returns 400 when pathName missing", async () => {
        const res = makeRes();
        await runRoute("post", "/:agentId/edit-transition", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { transitionCondition: "x" } }), res);
        expect(res._status).toBe(400);
    });
    it("returns 400 when transitionCondition empty", async () => {
        const res = makeRes();
        await runRoute("post", "/:agentId/edit-transition", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { pathName: "Sales", transitionCondition: "" } }), res);
        expect(res._status).toBe(400);
    });
    it("returns 404 when agent not found", async () => {
        mockGetClientDocument.mockResolvedValue(null);
        const res = makeRes();
        await runRoute("post", "/:agentId/edit-transition", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { pathName: "Sales", transitionCondition: "x" } }), res);
        expect(res._status).toBe(404);
    });
    ...
    it("updates the edge prompt and audits on success", async () => {
        ...
        const res = makeRes();
        await runRoute("post", "/:agentId/edit-transition", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { pathName: "Sales", transitionCondition: "user wants Sales" } }), res);
        expect(res._status).toBe(200);
        expect(res._json).toEqual({ success: true, pathName: "Sales" });
        expect(edge.transition_condition.prompt).toBe("user wants Sales");
        expect(mockLogAudit).toHaveBeenCalledWith(expect.anything(), "edit_transition", "acme/agent_1", { pathName: "Sales", transitionCondition: "user wants Sales" });
    });
...
describe("POST /:agentId/edit-agent-settings", () => {
    it("returns 400 when no allowed settings provided", async () => {
        const res = makeRes();
        await runRoute("post", "/:agentId/edit-agent-settings", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { not_allowed: "x" } }), res);
        expect(res._status).toBe(400);
    ...
    it("filters out disallowed keys, keeps allowed", async () => {
        ...
        mockFetchRetellAgent
            ...
            .mockResolvedValueOnce({ canonicalJson: { conversationFlow: {} }, conversationFlowId: "f1", agentName: "A" });
        const res = makeRes();
        await runRoute("post", "/:agentId/edit-agent-settings", makeReq({
    ...
    it("returns 404 when agent not found", async () => {
        mockGetClientDocument.mockResolvedValue(null);
        const res = makeRes();
        await runRoute("post", "/:agentId/edit-agent-settings", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { agent_name: "X" } }), res);
        expect(res._status).toBe(404);
    });
    it("audits with the list of fields updated", async () => {
        ...
        const res = makeRes();
        await runRoute("post", "/:agentId/edit-agent-settings", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { language: "en-US" } }), res);
        expect(res._status).toBe(200);
        expect(mockLogAudit).toHaveBeenCalledWith(expect.anything(), "edit_agent_settings", "acme/agent_1", { fields: ["language"] });
    });
