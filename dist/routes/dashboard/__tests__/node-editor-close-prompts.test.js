...
const { mockAgentUpdate, mockFlowUpdate, mockGetClientDocument, mockLoadClientsFromDb, mockGetDb, mockUpdateOne, mockFetchRetellAgent, mockPushFlowToRetell, mockExtractVariables, mockParseConversationFlow, mockValidateConversationFlow, mockCreateVersionSnapshot, mockLogAudit, mockDeriveNotificationConfig, mockRequireRoot, mockGetDataPointDefaults, mockResolveDataPoints, mockMakeIdFactory, mockBuildTransitionNode, mockBuildDataChain, mockBuildWarmTransferOption, mockGetWarmTransferAgentVersion, mockRenderTemplate, mockReplaceBusinessName, } = vi.hoisted(() => ({
...
vi.mock("../../../lib/agent-generator/node-builders.js", () => ({
    makeIdFactory: (...a) => mockMakeIdFactory(...a),
    buildTransitionNode: (...a) => mockBuildTransitionNode(...a),
    buildDataChain: (...a) => mockBuildDataChain(...a),
...
vi.mock("../../../lib/replace-business-name.js", () => ({
    replaceBusinessName: (...a) => mockReplaceBusinessName(...a),
}));
...
describe("POST /:agentId/save-and-publish — per-path Close prompts", () => {
    it("updates per-path Close (pathName) nodes when pathClosePrompts is provided", async () => {
        ...
        const nodes = [routerSvc, routerSls, closeService, closeSales, { id: "cr", name: "Closing Remarks" }];
    ...
    it("lazy-migrates a legacy singleton Close into per-path Close nodes when prompts diverge", async () => {
        ...
        const nodes = [routerSvc, routerSls, legacyClose, { id: "cr", name: "Closing Remarks" }];
        mockFetchRetellAgent.mockResolvedValue({
            canonicalJson: { conversationFlow: { nodes } }, conversationFlowId: "f1", agentName: "Acme",
        });
        mockParseConversationFlow.mockReturnValue({
    ...
    it("single-path agents update the singleton Close in place (no rename)", async () => {
        ...
        const nodes = [router, close, { id: "cr", name: "Closing Remarks" }];
        mockFetchRetellAgent.mockResolvedValue({
            canonicalJson: { conversationFlow: { nodes } }, conversationFlowId: "f1", agentName: "Acme",
        });
        mockParseConversationFlow.mockReturnValue({
        ...
        expect(nodes.find((n) => n.name === "Close (Default)")).toBeUndefined();
    });
    it("legacy changes.closePrompt (single string) applies to every callback path", async () => {
        ...
        const routerSvc = { id: "router-svc", else_edge: { destination_node_id: "close-svc" } };
        const routerSls = { id: "router-sls", else_edge: { destination_node_id: "close-sls" } };
        const nodes = [routerSvc, routerSls, closeSvc, closeSls, { id: "cr", name: "Closing Remarks" }];
        mockFetchRetellAgent.mockResolvedValue({
            canonicalJson: { conversationFlow: { nodes } }, conversationFlowId: "f1", agentName: "Acme",
        });
        mockParseConversationFlow.mockReturnValue({
    ...
    it("transfer paths in pathClosePrompts are ignored (no Close node to update)", async () => {
        ...
        const nodes = [routerSvc, routerEm, closeSvc, ptEm, { id: "cr", name: "Closing Remarks" }];
        mockFetchRetellAgent.mockResolvedValue({
            canonicalJson: { conversationFlow: { nodes } }, conversationFlowId: "f1", agentName: "Acme",
        });
        mockParseConversationFlow.mockReturnValue({
        ...
        expect(nodes.find((n) => n.name === "Close (Emergency)")).toBeUndefined();
    });
});
