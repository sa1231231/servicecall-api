...
const { mockAgentUpdate, mockFlowUpdate, mockPhoneList, mockPhoneUpdate, mockGetClientDocument, mockLoadClientsFromDb, mockGetDb, mockUpdateOne, mockFetchRetellAgent, mockPushFlowToRetell, mockExtractVariables, mockParseConversationFlow, mockValidateConversationFlow, mockCreateVersionSnapshot, mockLogAudit, mockDeriveNotificationConfig, mockRequireRoot, } = vi.hoisted(() => ({
...
describe("POST /:agentId/rename-business", () => {
    it("returns 400 when newName is missing", async () => {
        const res = makeRes();
        await runRoute("post", "/:agentId/rename-business", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: {} }), res);
        expect(res._status).toBe(400);
        expect(res._json.error).toMatch(/newName/);
    });
    it("returns 400 when newName is whitespace", async () => {
        const res = makeRes();
        await runRoute("post", "/:agentId/rename-business", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { newName: "   " } }), res);
        expect(res._status).toBe(400);
    });
    it("returns 404 when agent not found", async () => {
        mockGetClientDocument.mockResolvedValue(null);
        const res = makeRes();
        await runRoute("post", "/:agentId/rename-business", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { newName: "Beta" } }), res);
    ...
    it("returns success with unchanged=true when names match", async () => {
        ...
        const res = makeRes();
        await runRoute("post", "/:agentId/rename-business", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { newName: "Acme Plumbing" } }), res);
    ...
    it("returns 400 when validation fails on the renamed flow", async () => {
        ...
        const res = makeRes();
        await runRoute("post", "/:agentId/rename-business", makeReq({ params: { slug: "acme", agentId: "agent_1" }, body: { newName: "Beta Plumbing" } }), res);
        expect(res._status).toBe(400);
    ...
    it("snapshots, replaces flow, updates client.name, audits — without touching agent_name or phone nicknames", async () => {
        ...
        const res = makeRes();
        await runRoute("post", "/:agentId/rename-business", makeReq({
        ...
        expect(mockUpdateOne).toHaveBeenCalledWith({ _id: "acme" }, { $set: { name: "Beta Plumbing" } });
        // Audit logged.
    ...
    it("does not touch Retell phone numbers — even when the agent has many bound numbers", async () => {
        mockGetClientDocument.mockResolvedValue(makeDoc({ name: "Acme Plumbing", outbound_from_number: "+15550000000" }));
    ...
    it("preserves the existing display_name in agent_name pin when one is set", async () => {
        mockGetClientDocument.mockResolvedValue(makeDoc({ name: "Acme Plumbing", display_name: "Acme HVAC (demo)" }));
        mockFetchRetellAgent
            .mockResolvedValueOnce(makeSnapshot("Acme Plumbing"))
            .mockResolvedValueOnce(makeSnapshot("Beta Plumbing"));
        const res = makeRes();
