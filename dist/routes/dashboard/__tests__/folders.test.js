...
const { mockFoldersFind, mockFoldersFindOne, mockFoldersInsertOne, mockFoldersFindOneAndUpdate, mockFoldersDeleteOne, mockClientsUpdateMany, mockClientsUpdateOne, mockGetClientDocument, mockUpdateClientFields, mockLoadClientsFromDb, } = vi.hoisted(() => ({
    ...
    getDb: () => ({
        collection: (name) => {
            if (name === "agent_folders") {
                return {
                    find: (...a) => mockFoldersFind(...a),
                    findOne: (...a) => mockFoldersFindOne(...a),
                    insertOne: (...a) => mockFoldersInsertOne(...a),
                    findOneAndUpdate: (...a) => mockFoldersFindOneAndUpdate(...a),
                    deleteOne: (...a) => mockFoldersDeleteOne(...a),
                };
            }
            if (name === "clients") {
                return {
                    updateMany: (...a) => mockClientsUpdateMany(...a),
                    updateOne: (...a) => mockClientsUpdateOne(...a),
                };
            }
...
function makeCursor(toArrayResult, nextResult) {
    const c = {
...
describe("PATCH /dashboard/api/folders/:id", () => {
    it("400 on invalid id", async () => {
        const res = makeRes();
        await folders.updateFolderHandler(makeReq({ params: { id: "not-an-objectid" }, body: { name: "X" } }), res);
        expect(res._status).toBe(400);
    });
    it("400 when no supported fields are provided", async () => {
        const res = makeRes();
        await folders.updateFolderHandler(makeReq({ params: { id: new ObjectId().toString() }, body: { foo: "bar" } }), res);
        expect(res._status).toBe(400);
    });
    it("400 when name is empty string", async () => {
        const res = makeRes();
        await folders.updateFolderHandler(makeReq({ params: { id: new ObjectId().toString() }, body: { name: "  " } }), res);
        expect(res._status).toBe(400);
    });
    it("404 when folder does not exist", async () => {
        mockFoldersFindOneAndUpdate.mockResolvedValue(null);
        const res = makeRes();
        await folders.updateFolderHandler(makeReq({ params: { id: new ObjectId().toString() }, body: { name: "X" } }), res);
    ...
    it("renames folder and returns the updated doc", async () => {
        ...
        const res = makeRes();
        await folders.updateFolderHandler(makeReq({ params: { id: id.toString() }, body: { name: "New Name" } }), res);
        expect(res._status).toBe(200);
    ...
    it("accepts numeric position update", async () => {
        ...
        const res = makeRes();
        await folders.updateFolderHandler(makeReq({ params: { id: id.toString() }, body: { position: 7 } }), res);
        expect(res._status).toBe(200);
...
describe("DELETE /dashboard/api/folders/:id", () => {
    ...
    it("404 when folder does not exist", async () => {
        ...
        const res = makeRes();
        await folders.deleteFolderHandler(makeReq({ params: { id: new ObjectId().toString() } }), res);
        expect(res._status).toBe(404);
    });
    it("clears folder_id on contained agents (does NOT delete them) then drops the folder", async () => {
        ...
        expect(mockClientsUpdateMany).toHaveBeenCalledWith({ folder_id: id.toString() }, { $set: { folder_id: null } });
...
describe("PATCH /dashboard/api/agents/:slug/folder", () => {
    it("404 when client not found", async () => {
        ...
        const res = makeRes();
        await moveAgentFolder.moveAgentFolderHandler(makeReq({ params: { slug: "missing" }, body: { folder_id: null } }), res);
        expect(res._status).toBe(404);
    });
    it("400 on malformed folder_id", async () => {
        mockGetClientDocument.mockResolvedValue({ _id: "acme" });
        const res = makeRes();
        await moveAgentFolder.moveAgentFolderHandler(makeReq({ params: { slug: "acme" }, body: { folder_id: "bogus" } }), res);
        expect(res._status).toBe(400);
    });
    it("400 when folder_id refers to a non-existent folder", async () => {
        ...
        const res = makeRes();
        await moveAgentFolder.moveAgentFolderHandler(makeReq({ params: { slug: "acme" }, body: { folder_id: new ObjectId().toString() } }), res);
        expect(res._status).toBe(400);
    ...
    it("moves to root when folder_id is null", async () => {
        ...
        const res = makeRes();
        await moveAgentFolder.moveAgentFolderHandler(makeReq({ params: { slug: "acme" }, body: { folder_id: null } }), res);
        expect(res._status).toBe(200);
    ...
    it("moves to a folder when folder_id refers to an existing folder", async () => {
        ...
        mockUpdateClientFields.mockResolvedValue(undefined);
        const res = makeRes();
        await moveAgentFolder.moveAgentFolderHandler(makeReq({ params: { slug: "acme" }, body: { folder_id: folderId.toString() } }), res);
        expect(res._status).toBe(200);
