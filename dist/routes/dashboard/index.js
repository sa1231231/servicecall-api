...
import { listUsers, createUser, deleteUser, updateUserPermissions, PERMISSION_DEFS, DEFAULT_PERMISSIONS, } from "../../lib/users.js";
...
dashboardApiRouter.post("/agents/:slug/send-payment-link", requirePermission("send_comms"), async (req, res) => {
    ...
    const message = settings.payment_sms_message.replace(/\{\{stripe_payment_url\}\}/g, settings.stripe_payment_url);
    try {
        await sendSmsToAll(numbers, message);
        res.json({ success: true, sent_to: numbers });
    }
...
dashboardApiRouter.post("/agents/:slug/send-portal-link", requirePermission("send_comms"), async (req, res) => {
    ...
    const message = settings.portal_sms_message.replace(/\{\{portal_url\}\}/g, portalUrl);
    try {
        await sendSmsToAll(numbers, message);
        res.json({ success: true, sent_to: numbers });
    }
