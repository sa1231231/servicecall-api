...
const { mockGetClientDocument, mockRunSmokeTest, mockBuildSyntheticVariables, } = vi.hoisted(() => ({
...
vi.mock("../../lib/qa-smoke.js", () => ({
    runSmokeTest: (...a) => mockRunSmokeTest(...a),
    buildSyntheticVariables: (...a) => mockBuildSyntheticVariables(...a),
}));
...
beforeEach(() => {
    for (const m of [mockGetClientDocument, mockRunSmokeTest, mockBuildSyntheticVariables])
        m.mockReset();
});
// ── healthRouter ──────────────────────────────────────────────────────────
describe("GET /health", () => {
...
describe("POST /qa/smoke/:slug", () => {
    ...
    it("runs smoke test and returns the report", async () => {
        ...
        const res = makeRes();
        await runRoute(qaRouter, "post", "/smoke/:slug", makeReq({ params: { slug: "x" }, query: { notify: "true" }, host: "app.example.com" }), res);
...
describe("POST /qa/test-notify/:slug", () => {
    ...
    it("posts synthetic call to post-hook and returns success on 2xx", async () => {
        ...
        const res = makeRes();
        await runRoute(qaRouter, "post", "/test-notify/:slug", makeReq({ params: { slug: "x" }, host: "app.example.com" }), res);
        expect(res._json).toMatchObject({ success: true, outcome: "dispatched", shadow_mode: false });
        expect(fetchMock).toHaveBeenCalledWith("https://app.example.com/retell/post-hook", expect.objectContaining({ method: "POST" }));
        vi.unstubAllGlobals();
    });
