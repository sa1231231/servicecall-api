...
export function buildWarmTransferOption(agentVersion) {
    return {
...
Do NOT leave this node if the caller is only asking questions. Let the Admin/FAQ global node handle those and return here.`,
        ...
        finetune_transition_examples: [
            ...resolveFinetuneExamples(config.introFinetuneExamples, ids.paths[0].transitionId, nodeMap, f),
...
export function buildTransferCallNode(ids, pos, f, warmTransferAgentVersion) {
    return {
        custom_sip_headers: {},
...
export function buildPerPathTransferCallNode(pathIds, pathPos, ids, resolvedNumber, pathLabel, f, warmTransferAgentVersion) {
...
export function buildLiveTransferRecoveryNode(agentConfig, ids, pos, f) {
    const template = agentConfig.liveTransferRecoveryPrompt ??
...
export function buildCloseNode(agentConfig, ids, pos, f, overrides) {
    ...
    const template = overrides?.promptText ??
        (overrides?.pathName
