...
import { CALLER_DOESNT_KNOW, defaultExtractEquation, NOT_MENTIONED, } from "../agent-generator/data-point-registry.js";
function goodString(name) {
    return {
        label: name,
        variableName: name,
        type: "string",
...
describe("lintDataPoint", () => {
    describe("string data points", () => {
        ...
        it("flags extractSuccessEquation that doesn't reference its own variable", () => {
            ...
            expect(lintDataPoint(dp).map((e) => e.code)).toContain("EXTRACT_EQUATION_DOES_NOT_REFERENCE_SELF");
        });
    ...
    describe("finetune examples", () => {
        it("flags invalid type", () => {
            const dp = goodString("full_name");
            dp.finetuneExamples = [
                { type: "wrong", transcript: [{ content: "x", role: "user" }] },
            ];
...
describe.skipIf(!MONGODB_URL)("Deployed data point defaults (live MongoDB)", () => {
    ...
    it("every deployed data point default lints clean", async () => {
        ...
        for (const doc of docs) {
            ...
            const errors = lintDataPoint(dp);
            if (errors.length > 0)
                errorsByVar[dp.variableName] = errors;
        }
        if (Object.keys(errorsByVar).length > 0) {
            const lines = ["Deployed data point lint errors:"];
    ...
    it("every deployed agent's canonical flow has no dangling branch variable references", async () => {
        const versions = db.collection("agent_versions");
        ...
        for (const v of latest) {
            ...
            const errors = lintBranchVariableReferences(flow);
            if (errors.length > 0)
                errorsByAgent[`${v.slug}/${v.agentId}`] = errors;
        }
        if (Object.keys(errorsByAgent).length > 0) {
            const lines = ["Branch variable lint errors per agent:"];
