...
describe("deriveNotificationConfig", () => {
    ...
    it("builds subject template from name, address, city", () => {
        ...
        const result = deriveNotificationConfig(vars, baseClient, "agent_123");
        expect(result.message_types.service_request.subject_template).toBe("Service Request: {{full_name}} — {{street_address}}, {{city}}");
    });
    it("builds subject template with only name", () => {
        const vars = [{ key: "full_name", label: "Name" }];
        const result = deriveNotificationConfig(vars, baseClient, "agent_123");
        expect(result.message_types.service_request.subject_template).toBe("Service Request: {{full_name}}");
    });
    it("builds empty subject template when no name/address/city", () => {
        ...
        const result = deriveNotificationConfig(vars, baseClient, "agent_123");
        expect(result.message_types.service_request.subject_template).toBe("Service Request:");
    });
    it("filters out phone_number_collected from fields", () => {
        ...
        const result = deriveNotificationConfig(vars, baseClient, "agent_123");
        const fieldKeys = result.message_types.service_request.fields.map((f) => f.key);
    ...
    it("creates emergency + service_request when is_emergency present", () => {
        ...
        const emergencyKeys = result.message_types.emergency.fields.map((f) => f.key);
        ...
        expect(result.message_types.emergency.additional_text).toContain("10 minutes");
        expect(result.message_types.emergency.subject_template).toMatch(/^EMERGENCY:/);
    ...
    it("emergency fields fall back to all fields when no critical keys match", () => {
        ...
        expect(result.message_types.emergency.fields.length).toBe(result.message_types.service_request.fields.length);
    });
...
describe("deriveMultiPathNotificationConfig", () => {
    ...
    it("builds subject template per path", () => {
        ...
        const result = deriveMultiPathNotificationConfig(pathVars, baseClient, "agent_123");
        expect(result.message_types.emergency.subject_template).toBe("Emergency: {{full_name}} — {{street_address}}, {{city}}");
    });
