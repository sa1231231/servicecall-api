...
describe("saveCallLog", () => {
    it("upserts via replaceOne keyed by _id", async () => {
        ...
        expect(mockReplaceOne).toHaveBeenCalledWith({ _id: "call_abc" }, doc, { upsert: true });
    });
    it("swallows errors (fire-and-forget)", async () => {
        ...
        expect(err).toHaveBeenCalledWith(expect.stringContaining("failed to save call"), "conn lost");
        err.mockRestore();
    });
});
describe("enrichCallLog", () => {
    it("filters undefined/null values before $set", async () => {
        ...
        expect(mockUpdateOne).toHaveBeenCalledWith({ _id: "call_x" }, { $set: { call_summary: "ok", transcript: "hello" } });
    });
