...
describe("replaceBusinessName", () => {
    it("replaces top-level string fields", () => {
        const out = replaceBusinessName({ agent_name: "Acme Plumbing", agent_id: "agent_1" }, "Acme Plumbing", "Beta Plumbing");
    ...
    it("replaces inside nested prompt strings", () => {
        ...
        }, "Acme Plumbing", "Beta Plumbing");
        const flow = out.conversationFlow;
        expect(flow.global_prompt).toContain("Beta Plumbing");
        expect(flow.global_prompt).not.toContain("Acme Plumbing");
        const nodes = flow.nodes;
        expect(nodes[0].instruction.text).toContain("Beta Plumbing");
    });
    it("is case-insensitive on match but uses the new name's casing in replacement", () => {
        const out = replaceBusinessName({ text: "ACME plumbing and acme PLUMBING" }, "Acme Plumbing", "Beta Plumbing");
        expect(out.text).toBe("Beta Plumbing and Beta Plumbing");
    });
    it("escapes regex metacharacters in the old name", () => {
        const out = replaceBusinessName({ text: "Bob's HVAC & Heating (LLC)" }, "Bob's HVAC & Heating (LLC)", "Acme Co");
    ...
    it("preserves non-matching fields verbatim", () => {
        const out = replaceBusinessName({ agent_name: "Acme", llm_id: "abc-123", voice_speed: 1.0 }, "Acme", "Beta");
    ...
    it("does not affect the original key names — only string values", () => {
        ...
        const out = replaceBusinessName({ Acme: "value" }, "Acme", "Beta");
