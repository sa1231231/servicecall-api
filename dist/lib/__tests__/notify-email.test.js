...
describe("sendEmail", () => {
    ...
    it("falls back to text body when html is omitted", async () => {
        ...
        expect(mockSend).toHaveBeenCalledWith(expect.objectContaining({ text: "plain only" }));
    ...
    it("throws on resend error", async () => {
        ...
        await expect(sendEmail({ to: "u@x.com", subject: "s", body: "b" })).rejects.toThrow("Resend error: blocked");
    });
});
