...
describe("logPhoneEvent", () => {
    ...
    it("swallows insert errors (fire-and-forget)", async () => {
        ...
        await expect(logPhoneEvent("acme", "+15550001111", "PN", "provisioned")).resolves.toBeUndefined();
        expect(err).toHaveBeenCalledWith(expect.stringContaining("failed to log provisioned"), "conn lost");
        err.mockRestore();
    });
});
describe("getNumberDaysInRange", () => {
    it("returns 0 when there are no events for the client", async () => {
        ...
        const days = await getNumberDaysInRange("acme", D("2026-01-01T00:00:00Z"), D("2026-02-01T00:00:00Z"));
        expect(days).toBe(0);
    });
...
describe("backfillCurrentPhoneNumbers", () => {
    it("inserts a synthetic provisioned event when none exists", async () => {
        ...
        const written = await backfillCurrentPhoneNumbers([{ client_slug: "acme", phone_number: "+15550001111", phone_number_sid: "PN_a" }], at);
