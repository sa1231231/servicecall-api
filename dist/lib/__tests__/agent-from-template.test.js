...
const { slugify, loadTemplate, applyOverrides } = await import("../agent-from-template.js");
// ── slugify ────────────────────────────────────────────────────────────────
describe("slugify", () => {
    ...
    it("matches the UI's slug regex (public/index.html:1922)", () => {
        const ui = (s) => s.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "");
