...
function generateSinglePath(dpKeys) {
...
function dataPointsFromChain(chain) {
    return chain.map((dp) => ({
        label: dp.label,
        variableName: dp.variableName,
        type: dp.variableDefs[0]?.type ?? "string",
...
describe("orphan data points", () => {
    it("generate agent with orphan DP — no Collect/Confirm nodes for it", () => {
        ...
        expect(nodeNames.some((n) => n.includes("Full Name") && n.includes("Collect"))).toBe(true);
        expect(nodeNames.some((n) => n.includes("Is Loaded") && n.includes("Collect"))).toBe(false);
    });
    it("orphan variable appears in front extract node", () => {
        ...
        const nodes = agent.conversationFlow.nodes;
        const extractNode = nodes.find((n) => n.type === "extract_dynamic_variables" && n.name.includes("Extract"));
        expect(extractNode).toBeDefined();
        const varNames = extractNode.variables.map((v) => v.name);
    ...
    it("orphan variable not in router edges", () => {
        ...
        const nodes = agent.conversationFlow.nodes;
        const router = nodes.find((n) => n.type === "branch" && n.name.includes("Router"));
    ...
    it("validates successfully with orphan data point", () => {
        ...
        expect(validateConversationFlow(agent.conversationFlow)).toEqual([]);
    });
...
describe("validateConversationFlow", () => {
    describe("valid flows pass", () => {
        ...
        it("agent with branch conditions", () => {
            const { agent } = generateAgent(baseConfig, [
                "vehicle_type",
                { _branch: true, variable: "vehicle_type", operator: "==", value: "Semi",
    ...
    describe("catches type-specific errors", () => {
        ...
        it("extract node without variables", () => {
            const { agent } = generateSinglePath(["full_name"]);
            const flow = agent.conversationFlow;
            const extractNode = flow.nodes.find((n) => n.type === "extract_dynamic_variables");
        ...
        it("extract node with duplicate variable names", () => {
            const { agent } = generateSinglePath(["full_name"]);
            const flow = agent.conversationFlow;
            const extractNode = flow.nodes.find((n) => n.type === "extract_dynamic_variables" && n.name.startsWith("Confirm"));
        ...
        it("branch node without edges", () => {
            const { agent } = generateSinglePath(["full_name"]);
            const flow = agent.conversationFlow;
            const router = flow.nodes.find((n) => n.type === "branch");
            router.edges = [];
        ...
        it("transfer node without destination", () => {
            const { agent } = generateAgent({ ...baseConfig, humanRequestMode: "live_transfer" }, ["full_name"], undefined, TEST_DEFAULTS);
            const flow = agent.conversationFlow;
            const transfer = flow.nodes.find((n) => n.type === "transfer_call");
...
describe("regenerateDataChain", () => {
    ...
    describe("multi-path operations", () => {
        ...
        it("adds path name suffix to node names", () => {
            ...
            const result = regenerateDataChain(residential, dps, parsed.closeNode.id, "Residential");
            const routerNode = result.newNodes.find(n => n.type === "branch");
            expect(routerNode?.name).toContain("(Residential)");
            const extractNode = result.newNodes.find(n => n.type === "extract_dynamic_variables" &&
                n.name.startsWith("Extract All"));
            expect(extractNode?.name).toContain("(Residential)");
        });
    });
    describe("special data point handling", () => {
        it("handles phone_number with collected flag", () => {
            ...
            expect(eqs.some((eq) => eq.left === "{{phone_number_collected}}")).toBe(true);
        });
...
describe("round-trip integrity", () => {
    ...
    it("front extract node contains all variables after add", () => {
        ...
        applyRegeneratedChain(agent, result);
        const flow = agent.conversationFlow;
        const extractAll = flow.nodes.find((n) => n.name === "Extract All Variables");
        const varNames = extractAll.variables.map((v) => v.name);
        expect(varNames).toContain("full_name");
    ...
    it("confirm nodes have tapered variable lists", () => {
        ...
        const confirmNodes = result.newNodes.filter(n => n.type === "extract_dynamic_variables" && n.name.startsWith("Confirm"));
        const firstConfirm = confirmNodes.find(n => n.name === "Confirm Full Name");
        const lastConfirm = confirmNodes.find(n => n.name === "Confirm City");
        expect(firstConfirm.variables.length).toBeGreaterThanOrEqual(3);
        expect(lastConfirm.variables.length).toBeGreaterThanOrEqual(1);
        // Last confirm should have fewer variables than first
        expect(lastConfirm.variables.length).toBeLessThan(firstConfirm.variables.length);
    });
});
...
describe("regenerator: respects per-path end mode", () => {
    it("rewires variables router → existing pre-transfer (not close) when path is transfer-mode", () => {
        ...
        const router = result.newNodes.find((n) => n.name === "Variables Router (Emergency)");
