...
export function gatherRecipients() {
    const recipients = [];
    let clientCount = 0;
    for (const client of Object.values(notificationClients)) {
        if (client.shadow_mode === true)
            continue;
        if (client.active === false)
            continue;
        if (!client.dispatch_text_numbers || client.dispatch_text_numbers.length === 0)
            continue;
        clientCount++;
...
export function personalizeMessage(template, clientName) {
    return template.replace(/\{\{client_name\}\}/gi, clientName);
}
/** Preview the blast without sending. */
export function previewBlast(message) {
...
export async function sendBlast(message) {
    const { recipients, clientCount } = gatherRecipients();
    const result = {
        total_recipients: recipients.length,
        total_clients: clientCount,
