...
import { makeIdFactory } from "./agent-generator/node-builders.js";
// ── Helpers ──────────────────────────────────────────────────────────────────
function toVarDefs(dp) {
 ...
 */
export function regenerateDataChain(existingPath, newDataPoints, closeNodeId, pathName) {
    ...
    const terminalNodeId = existingPath.endMode === "transfer" && existingPath.preTransferNode
    ...
    const chainIds = newDataPoints.map((dp) => {
    ...
    const BASE_X = -954;
    const STEP_X = 550;
    const existingFrontExtractPos = existingPath.frontExtractNode.raw.display_position ?? { x: -18, y: 0 };
    const existingRouterPos = existingPath.routerNode.raw.display_position ?? { x: -18, y: 450 };
    ...
    newDataPoints.forEach((dp, i) => {
        if (dp.orphan)
            return; // Extract-only: no Collect+Confirm nodes
        ...
        const convPos = existing
            ? existing.collectNode.raw.display_position
            : { x: BASE_X + i * STEP_X, y: yBase + 900 };
        const confirmPos = existing
            ? existing.confirmNode.raw.display_position
        ...
        const finetuneConv = existingCollectRaw?.finetune_conversation_examples ?? [];
        const finetuneTrans = existingCollectRaw?.finetune_transition_examples ?? [];
        nodes.push({
        ...
        const confirmFinetune = existingConfirmRaw?.finetune_transition_examples ?? [];
        nodes.push({
 ...
 */
export function applyRegeneratedChain(canonicalJson, result) {
    const flow = canonicalJson.conversationFlow;
    const nodes = flow.nodes;
    // Remove old chain nodes
    const filtered = nodes.filter((n) => !result.removedNodeIds.has(n.id));
    // Add new chain nodes
