...
const {
  ...
  mockGetClientDocument, mockPersistClient,
...
function makeReq(opts: { params?: any; body?: any }): Request {
  return { params: opts.params ?? {}, body: opts.body ?? {} } as any;
}
...
beforeEach(() => {
  ...
  mockGenerateSlug.mockImplementation((name: string) => name.toLowerCase().replace(/\s+/g, "-"));
...
describe("cloneAgentHandler", () => {
  ...
  it("clones successfully: creates flow + agent, persists client, returns 201", async () => {
    ...
    const greeting = flowParamsCall.nodes.find((n: any) => n.name === "Greeting");
    expect(greeting.instruction.text).toMatch(/New Co/);
    expect(greeting.instruction.text).not.toMatch(/Old/);

    // FAQ should be replaced
    const faqNode = flowParamsCall.nodes.find((n: any) => n.name === "Admin/FAQ");
