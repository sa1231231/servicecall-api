...
vi.mock("../../../lib/blast-sms.js", () => ({
  previewBlast: (...args: any[]) => mockPreviewBlast(...args),
  sendBlast: (...args: any[]) => mockSendBlast(...args),
}));
...
vi.mock("../../../lib/root-alerts.js", () => ({
  alertRootIfNeeded: (...args: any[]) => mockAlertRoot(...args),
}));
...
function previewHandler(req: Request, res: Response) {
...
function mockReq(body: any): Request {
  return { body, user: { username: "admin" } } as any;
}
...
describe("blast-sms send handler", () => {
  ...
  it("sends blast, logs audit, and alerts root", async () => {
    ...
    expect(mockLogAudit).toHaveBeenCalledWith(
      ...
      "blast_sms",
      "global",
      expect.objectContaining({ recipients: 3, sent: 3 }),
    );
