...
type PhoneEntry = { phone_number: string; nickname: string | null; role: Role };

export async function listPhoneNumbersHandler(
  _req: Request,
  res: Response,
): Promise<void> {
  ...
  const byAgent: Record<string, PhoneEntry[]> = {};
  ...
  for (const n of allNumbers) {
    ...
    for (const a of n.inbound_agents ?? []) {
      if (a.agent_id) inboundIds.add(a.agent_id);
    }
    ...
    const outboundAgents = (n as { outbound_agents?: Array<{ agent_id?: string }> })
      .outbound_agents ?? [];
    for (const a of outboundAgents) {
      if (!a.agent_id || inboundIds.has(a.agent_id)) continue;
