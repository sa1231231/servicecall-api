...
function col() {
  return getDb().collection<FolderDoc>(COLLECTION);
}
...
function serialize(doc: FolderDoc) {
  return {
...
export async function listFoldersHandler(_req: Request, res: Response): Promise<void> {
  try {
...
export async function deleteFolderHandler(req: Request, res: Response): Promise<void> {
  ...
  try {
    const cleared = await clientsCol().updateMany(
      { folder_id: id },
      { $set: { folder_id: null } },
    );
