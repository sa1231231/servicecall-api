...
function toLabel(variableName: string, dataPointLabel?: string): string {
...
function deriveNotificationConfig(
  resolved: DataPoint[],
  clientInfo: CreateAgentBody["client"],
  agentId: string,
): JsonClientEntry {
...
function extractFlowParams(conversationFlow: Record<string, unknown>) {
...
export async function createAgentHandler(
  ...
  try {
    ...
    const { agent: agentJson, resolved } = generateAgent(
      body.business,
      body.dataPoints,
    );
