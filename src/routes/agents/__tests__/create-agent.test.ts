...
describe("createAgentHandler — validation", () => {
  ...
  it("preserves the exact case of client.name through to deriveNotificationConfig", async () => {
    const res = mockRes();
    await createAgentHandler(
      mockReq(makeBody({ client: { slug: "gmc", name: "GMC" } })),
      res,
    );
    expect(res._status).toBe(201);
...
describe("createAgentHandler — success path", () => {
  ...
  it("writes the provisioned number to outbound_from_number on the client doc", async () => {
    ...
    expect(mockUpdateClientField).toHaveBeenCalledWith(
      "test-co",
  ...
  it("does NOT write outbound_from_number when provisioning fails", async () => {
    ...
    const calls = mockUpdateClientField.mock.calls.filter(
      (c) => c[1] === "outbound_from_number",
    );
