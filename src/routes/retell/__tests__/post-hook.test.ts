...
const { mockVerify, mockSendSms, mockSendSmsToAll, mockSendEmail, mockGetEmailStatus,
  mockSendOwnerCallMonitor, mockTriggerDispatchCall, mockSaveCallLog, mockCheckAgentAlerts,
  mockAgentIdToClient, mockAgentIdToSlug } = vi.hoisted(() => ({
  mockVerify: vi.fn(),
  mockSendSms: vi.fn(),
...
vi.mock("../../../lib/notify-sms.js", () => ({
  ...
  sendSmsToAll: (...a: any[]) => mockSendSmsToAll(...a),
}));

vi.mock("../../../lib/notify-email.js", () => ({
  sendEmail: (...a: any[]) => mockSendEmail(...a),
  getEmailStatus: (...a: any[]) => mockGetEmailStatus(...a),
}));

vi.mock("../../../lib/owner-monitor.js", () => ({
  sendOwnerCallMonitor: (...a: any[]) => mockSendOwnerCallMonitor(...a),
}));

vi.mock("../../../lib/dispatch-call.js", () => ({
  triggerDispatchCall: (...a: any[]) => mockTriggerDispatchCall(...a),
}));

vi.mock("../../../lib/call-log.js", () => ({
  saveCallLog: (...a: any[]) => mockSaveCallLog(...a),
}));

vi.mock("../../../lib/agent-alerts.js", () => ({
  checkAgentAlerts: (...a: any[]) => mockCheckAgentAlerts(...a),
}));
...
function makeBody(overrides: { event?: string; call?: Record<string, any> } = {}) {
...
describe("postHookHandler — signature verification", () => {
  ...
  it("skips signature verification for test clients", async () => {
    ...
    await postHookHandler(
      makeReq(makeBody({ call: { agent_id: "agent_test", from_number: null } })),
      res,
    );
  ...
  it("skips signature verification when x-api-key matches internal key", async () => {
    ...
    await postHookHandler(
      makeReq(makeBody(), { "x-api-key": "internal-api-key" }),
      res,
    );
...
describe("postHookHandler — web calls", () => {
  it("logs a web_call outcome and skips dispatch when from_number is missing", async () => {
    ...
    expect(mockSaveCallLog).toHaveBeenCalledWith(
      expect.objectContaining({ outcome: "web_call", from_number: "Web Call" }),
    );
  });
});
...
describe("postHookHandler — dispatch happy path", () => {
  it("sends SMS + email and logs dispatched outcome", async () => {
    ...
    expect(mockSendEmail).toHaveBeenCalledWith(
      expect.objectContaining({ to: "acme@x.com" }),
    );
    expect(mockSaveCallLog).toHaveBeenCalledWith(
      expect.objectContaining({ outcome: "dispatched", message_type_key: "default" }),
    );
    expect(mockSendOwnerCallMonitor).toHaveBeenCalledWith(
      expect.any(Object),
      expect.any(Object),
      "dispatched",
    );
  ...
  it("returns 500 when notification dispatch errors", async () => {
    ...
    expect(mockSaveCallLog).toHaveBeenCalledWith(
      expect.objectContaining({ outcome: "dispatch_error" }),
    );
  });
});
