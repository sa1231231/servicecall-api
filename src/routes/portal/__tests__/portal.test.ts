...
vi.mock("../../../config/client-store.js", () => ({
  validatePortalToken: (...a: any[]) => mockValidatePortalToken(...a),
  findClientsByEmail: (...a: any[]) => mockFindClientsByEmail(...a),
...
function makeRes(): Response & { _status: number; _json: any } {
  const res: any = { _status: 200, _json: null, _html: null };
  res.status = (code: number) => { res._status = code; return res; };
  res.json = (data: any) => { res._json = data; return res; };
  res.type = () => res;
  res.send = (data: any) => { res._html = data; return res; };
  return res;
}

function makeReq(opts: { params?: any; body?: any; query?: any; protocol?: string; host?: string }): Request {
...
function findRoute(method: string, path: string) {
  for (const layer of (portalRouter as any).stack as any[]) {
...
describe("POST /request-link", () => {
  ...
  it("sends links for each matched client (existing token reused)", async () => {
    ...
    const res = makeRes();
    await runRoute("post", "/request-link",
      makeReq({ body: { email: "alice@example.com" }, host: "app.example.com", protocol: "https" }), res);
    expect(res._status).toBe(200);
...
describe("portalGetAgentHandler", () => {
  function makePortalReq(slug: string) {
...
describe("portalGetCallsHandler", () => {
  function makePortalReq(slug: string, query: Record<string, any> = {}) {
...
describe("PATCH /:slug/api/settings", () => {
  ...
  it("returns 401 when token missing", async () => {
    ...
    const res = makeRes();
    await runRoute("patch", "/:slug/api/settings",
      makeReq({ params: { slug: "acme" }, body: { dispatch_email: ["a@b.com"] }, query: {} }), res);
  ...
  it("returns 401 when token invalid", async () => {
    ...
    const res = makeRes();
    await runRoute("patch", "/:slug/api/settings",
      makeReq({ params: { slug: "acme" }, body: { dispatch_email: ["a@b.com"] }, query: { token: "bad" } }), res);
    expect(res._status).toBe(401);
  });
  ...
  it("returns 404 when client not found", async () => {
    ...
    const res = makeRes();
    await runRoute("patch", "/:slug/api/settings",
      makeReq({ params: { slug: "acme" }, body: { dispatch_email: ["a@b.com"] }, query: { token: "t" } }), res);
    expect(res._status).toBe(404);
  });
  ...
  it("returns 400 when body empty", async () => {
    ...
    const res = makeRes();
    await runRoute("patch", "/:slug/api/settings",
      makeReq({ params: { slug: "acme" }, body: {}, query: { token: "t" } }), res);
