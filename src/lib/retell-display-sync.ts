...
export interface RetellDisplaySyncResult {
  ...
  // Twilio incoming-phone-number friendlyName updates. Kept in lockstep with
  // the Retell nickname so the Twilio console label tracks the Retell label
  // — when a number's nickname changes here, its friendlyName changes too.
...
export async function syncRetellDisplayLabels(
  ...
  let matching: Array<{ phone_number: string }> = [];
  try {
