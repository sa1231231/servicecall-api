...
import {
  getWarmTransferAgentVersion,
  _resetWarmTransferAgentVersionCache,
} from "../warm-transfer-agent-version.js";
import { WARM_TRANSFER_AGENT_VERSION_FALLBACK } from "../node-builders.js";

function makeRetell(retrieve: () => Promise<any>) {
  return { agent: { retrieve: vi.fn(retrieve) } } as any;
}
