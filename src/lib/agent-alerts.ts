...
const callSurgeLastAlerted = new Map<string, number>();

// ── Cost surge: daily accumulator per agent ──────────────────────────────────
const dailyCost = new Map<
  string,
  { date: string; totalCents: number; alerted: boolean }
>();

export interface AlertResult {
  callSurge: { fired: boolean; count: number };
  costSurge: { fired: boolean; totalCents: number };
}
...
export function checkAgentAlerts(
  agentId: string,
