...
describe("provisionPhoneNumber", () => {
  ...
  it("extracts area code from dispatch number", async () => {
    ...
    expect(mockAvailableList).toHaveBeenCalledWith(
      expect.objectContaining({ areaCode: 301, limit: 1 }),
    );
  });
  ...
  it("adds number to SIP trunk", async () => {
    ...
    expect(mockTrunkPhoneCreate).toHaveBeenCalledWith(
      expect.objectContaining({ phoneNumberSid: "PN_test_sid" }),
    );
  });
  ...
  it("adds number to messaging service", async () => {
    ...
    expect(mockMsgPhoneCreate).toHaveBeenCalledWith(
      expect.objectContaining({ phoneNumberSid: "PN_test_sid" }),
    );
  });
  ...
  it("throws when no numbers available", async () => {
    ...
    await expect(provisionPhoneNumber(defaultOptions)).rejects.toThrow(
      "No available phone numbers in area code 301",
    );
  });
  ...
  it("extracts area code from various formats", async () => {
    ...
    expect(mockAvailableList).toHaveBeenCalledWith(
      expect.objectContaining({ areaCode: 815 }),
    );
  });
