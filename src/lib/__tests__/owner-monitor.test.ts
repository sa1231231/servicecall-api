...
vi.mock("../call-log.js", () => ({
  enrichCallLog: (...args: any[]) => mockEnrichCallLog(...args),
}));

const { sendOwnerCallMonitor } = await import("../owner-monitor.js");

function makeClient(overrides: Partial<ClientNotificationConfig> = {}): ClientNotificationConfig {
  return {
    name: "Acme",
    agent_id: "agent_1",
...
async function runWithTimers(fn: () => Promise<unknown>) {
  const promise = fn();
...
describe("sendOwnerCallMonitor — happy path (non-problem call)", () => {
  ...
  it("enriches the call log with analysis data before sending email", async () => {
    ...
    expect(mockEnrichCallLog).toHaveBeenCalledWith(
      "call_x1",
...
describe("sendOwnerCallMonitor — formatting", () => {
  it("formats sub-minute durations correctly", async () => {
    ...
    await runWithTimers(() =>
      sendOwnerCallMonitor(shortCall, makeClient(), "dispatched"),
    );
  ...
  it("includes the notification outcome in both subject metadata and body", async () => {
    await runWithTimers(() =>
      sendOwnerCallMonitor(makeCall(), makeClient(), "skipped_required_field"),
    );
  ...
  it("omits links section when no recording or log url is present", async () => {
    mockRetellRetrieve.mockResolvedValue(
      makeCall({ recording_url: null, public_log_url: null }),
    );
    await runWithTimers(() =>
      sendOwnerCallMonitor(
        makeCall({ recording_url: null, public_log_url: null }),
        makeClient(),
        "dispatched",
      ),
