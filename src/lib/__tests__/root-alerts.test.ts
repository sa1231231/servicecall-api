...
function mockReq(user?: Request["user"]): Request {
  return { user, ip: "1.2.3.4" } as Request;
}
...
describe("alertRootIfNeeded", () => {
  it("skips alert when user is root", async () => {
    alertRootIfNeeded(
      mockReq({ username: "admin", role: "admin", permissions: {}, isRoot: true }),
      "delete_agent",
      "test-slug",
    );
  ...
  it("sends SMS and email for non-root user", async () => {
    alertRootIfNeeded(
      mockReq({ username: "sam", role: "admin", permissions: {}, isRoot: false }),
      "delete_agent",
      "acme-plumbing",
    );
    await new Promise((r) => setTimeout(r, 10));
  ...
  it("sends alert for operator performing action", async () => {
    alertRootIfNeeded(
      mockReq({ username: "jess", role: "operator", permissions: {}, isRoot: false }),
      "update_settings",
      "global",
  ...
  it("sends alert for viewer (if they somehow have permission)", async () => {
    alertRootIfNeeded(
      mockReq({ username: "va", role: "viewer", permissions: {}, isRoot: false }),
      "create_data_point",
      "lot_number",
    );
    await new Promise((r) => setTimeout(r, 10));
  ...
  it("does not throw when SMS/email fails", async () => {
    (sendSms as any).mockRejectedValueOnce(new Error("Twilio down"));
