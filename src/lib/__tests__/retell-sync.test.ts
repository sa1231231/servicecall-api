...
describe("extractVariables", () => {
  ...
  it("returns empty array when nodes is not an array", () => {
    expect(
      extractVariables({ conversationFlow: { nodes: "not-array" } }),
    ).toEqual([]);
  });
...
describe("pushFlowToRetell", () => {
  it("calls conversationFlow.update with stripped params", async () => {
    ...
    const retell = { conversationFlow: { update: mockUpdate } } as any;
  ...
  it("throws when conversationFlow is missing from canonical JSON", async () => {
    const retell = { conversationFlow: { update: vi.fn() } } as any;

    await expect(pushFlowToRetell(retell, "cf_x", {})).rejects.toThrow(
      "Missing conversationFlow",
    );
  });
  ...
  it("propagates errors from the SDK", async () => {
    ...
    await expect(
      pushFlowToRetell(retell, "cf_x", { conversationFlow: { nodes: [] } }),
...
describe("fetchRetellAgent", () => {
  function makeRetell(overrides: { agent?: any; flow?: any; agentError?: Error; flowError?: Error } = {}) {
    return {
      agent: {
  ...
  it("returns a snapshot with merged canonical JSON", async () => {
    ...
    expect(snap.canonicalJson.conversationFlow).toEqual(
      expect.objectContaining({ global_prompt: "Hi" }),
    );
  ...
  it("throws when agent response_engine is not conversation-flow type", async () => {
    ...
    await expect(fetchRetellAgent(retell, "agent_x")).rejects.toThrow(
      "does not use a conversation-flow response engine",
    );
  });
