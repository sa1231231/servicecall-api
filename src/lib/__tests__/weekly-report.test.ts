...
vi.mock("../db.js", () => ({
  getDb: () => ({
    collection: (name: string) => ({
      find: (filter: any) => ({
        toArray: async () => {
          if (name !== "call_logs") return [];
          return callLogDocs.filter((d: any) => {
            if (filter?.client_slug && d.client_slug !== filter.client_slug) return false;
            if (filter?.created_at) {
              if (d.created_at < filter.created_at.$gte) return false;
              if (d.created_at >= filter.created_at.$lt) return false;
            }
            return true;
          });
        },
...
describe("sendWeeklyReportForClient", () => {
  it("sends email and SMS with correct call counts", async () => {
    const ts = recentDate();
    callLogDocs.push(
      { client_slug: "test-client", message_type_key: "service_request", created_at: ts },
      { client_slug: "test-client", message_type_key: "service_request", created_at: ts },
      { client_slug: "test-client", message_type_key: "emergency", created_at: ts },
    );
  ...
  it("counts unknown message_type_key as Other", async () => {
    callLogDocs.push(
      { client_slug: "test-client", message_type_key: "unknown_type", created_at: recentDate() },
    );
  ...
  it("sends to multiple email recipients", async () => {
    await sendWeeklyReportForClient(
      makeClientDoc({ dispatch_email: ["a@test.com", "b@test.com"] }),
    );
  ...
  it("only counts calls within the past 7 days", async () => {
    ...
    callLogDocs.push(
      { client_slug: "test-client", message_type_key: "service_request", created_at: recent },
      { client_slug: "test-client", message_type_key: "service_request", created_at: eightDaysAgo },
    );
...
describe("runWeeklyReports", () => {
  ...
  it("skips docs with missing agent_id", async () => {
    vi.mocked(getAllClientDocuments).mockResolvedValue([
      makeClientDoc({ _id: "broken", agent_id: undefined as any }),
