...
const { buildLeadQueries, formatPreSearch, braveWebSearch, preSearchLead } = await import(
  "../brave-search.js"
);

// ── buildLeadQueries — pure ──────────────────────────────────────────────────

describe("buildLeadQueries", () => {
  it("emits three formats for an 11-digit US phone number", () => {
    const queries = buildLeadQueries({ phone: "+19739781542" });
  ...
  it("returns no queries when input is empty", () => {
    expect(buildLeadQueries({})).toEqual([]);
  });
});

// ── formatPreSearch — pure ───────────────────────────────────────────────────

describe("formatPreSearch", () => {
  it("returns empty string when no searches were run", () => {
    expect(formatPreSearch({ searches: [] })).toBe("");
  });

  it("renders each search with its query, hits, and an error / empty fallback", () => {
    const out = formatPreSearch({
      searches: [
        {
          ok: true,
          query: `"973-978-1542"`,
    ...
    expect(out).toContain("## Pre-search context");
...
// ── preSearchLead — runs every query sequentially ───────────────────────────

describe("preSearchLead", () => {
  const realFetch = global.fetch;
  beforeEach(() => { global.fetch = vi.fn() as any; });
  afterEach(() => { global.fetch = realFetch; });
  ...
  it("runs one search per query and returns them all", async () => {
    ...
    const out = await preSearchLead({ phone: "+19739781542", name: "Mario Mina" });
