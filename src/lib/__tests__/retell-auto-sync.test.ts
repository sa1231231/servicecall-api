...
vi.mock("../../config/client-store.js", () => ({
  getAllClientDocuments: (...a: any[]) => mockGetAllClientDocuments(...a),
...
function captureRunFn(): () => Promise<void> {
  const setIntervalSpy = vi.spyOn(global, "setInterval");
  startAutoSync();
  const fn = setIntervalSpy.mock.calls[0][0] as () => Promise<void>;
...
describe("runAutoSync (via captured interval fn)", () => {
  ...
  it("swallows snapshot errors without aborting the sync", async () => {
    ...
    expect(warn).toHaveBeenCalledWith(
      expect.stringContaining("could not snapshot drift"),
      expect.any(Error),
    );
