...
export function lintDataPoint(dp: DataPoint): LintError[] {
  const errors: LintError[] = [];
  const v = dp.variableName;
  const push = (code: string, message: string) =>
    errors.push({ code, message, variableName: v });

  if (!v || !v.trim()) {
    errors.push({ code: "NO_VARIABLE_NAME", message: "Data point has no variableName" });
    return errors;
  }
  if (!dp.label?.trim()) push("NO_LABEL", `"${v}" has empty label`);
...
function lintCompositeVariable(
  parent: string,
  sub: VariableDef,
  i: number,
  names: Set<string>,
...
function lintExtractEquation(v: string, eqs?: ExtractEquation[]): LintError[] {
...
function lintFinetuneExample(v: string, ex: FinetuneExample, i: number): LintError[] {
  const errors: LintError[] = [];
 ...
 * variable references.
 */
export function lintBranchVariableReferences(
  ...
  for (const node of nodes) {
    ...
    for (const v of vars) {
      ...
      if (name) extractedVars.add(name);
    }
  }
  ...
  for (const node of nodes) {
    ...
    for (let i = 0; i < edges.length; i++) {
      ...
      if (!tc || tc.type !== "equation") continue;
